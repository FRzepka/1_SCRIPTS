...
int main(void)
{
  ...
  MX_GPIO_Init();
  UART3_Init_Quick();
  UART3_SendString("BOOT INT8 LSTM FP32 MLP QUANTIZED\r\n");
  ...
  while (1){
    if (UART3_DataAvailable()){
      ...
      if (c=='\r' || c=='\n'){
        if (rxi>0){ rxbuf[rxi]=0; const char *p=rxbuf; float feat[INPUT_SIZE]; int ok=1; for (int i=0;i<INPUT_SIZE;i++){ if(!parse_float_simple(&p,&feat[i])){ ok=0; break; }}
          if (ok){
            lstm_reset_stack_measure(); // Reset stack tracker before run
            uint32_t t0 = DWT->CYCCNT;
            
            float y = lstm_model_soh_int8_forward(feat, &state);
            ...
            // QUANTIZED -> Blink LD2 (PE1) on each sample
    ...
    // Heartbeat also on LD2 (PE1) for consistency
    static uint32_t last=0; if (HAL_GetTick()-last>333){ last=HAL_GetTick(); HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_1);}    
  }
}
