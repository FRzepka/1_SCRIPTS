...
/* Stack Measurement */
volatile uint32_t g_min_stack_ptr = 0xFFFFFFFF;
static inline void update_stack_measure(void) {
    uint32_t sp;
    __asm volatile ("mov %0, sp" : "=r" (sp));
    if (sp < g_min_stack_ptr) g_min_stack_ptr = sp;
}
...
float lstm_model_soh_int8_forward(float input[INPUT_SIZE], LSTMStateSOH* state) {
  update_stack_measure();
  // Preprocess inputs using scaler header (in-place variant)
  float x_scaled[INPUT_SIZE];
  memcpy(x_scaled, input, sizeof(float)*INPUT_SIZE);
  scaler_soh_transform(x_scaled);
