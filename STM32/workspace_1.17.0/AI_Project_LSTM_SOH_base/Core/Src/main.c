...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body (FP32 SOH LSTM+MLP)
  ******************************************************************************
  */
...
#define BASE_FEATURES SCALER_NUM_FEATURES
...
int main(void)
{
  ...
  MX_GPIO_Init();
  UART3_Init_Quick();
  UART3_SendString("BOOT FP32 LSTM SOH BASE\r\n");
  ...
  while (1){
    if (UART3_DataAvailable()){
      ...
      if (c=='\r' || c=='\n'){
        if (rxi>0){ rxbuf[rxi]=0; const char *p=rxbuf; float feat[BASE_FEATURES]; int ok=1; for (int i=0;i<BASE_FEATURES;i++){ if(!parse_float_simple(&p,&feat[i])){ ok=0; break; }}
          if (ok){
            float x_scaled[BASE_FEATURES];
            // Apply scaler
            ...
            float y = 0.0f;
            lstm_model_soh_inference(&model, x_scaled, &y);
            ...
            // Blink LD1 (PB0 Green) on each sample
    ...
    // Heartbeat on LD1 (PB0)
    static uint32_t last=0; if (HAL_GetTick()-last>333){ last=HAL_GetTick(); HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);}    
  }
}
