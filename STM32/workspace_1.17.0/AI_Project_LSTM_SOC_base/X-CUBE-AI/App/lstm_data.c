...
ai_buffer g_lstm_data_map_activations[AI_LSTM_DATA_ACTIVATIONS_COUNT] = {
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 2584, 1, 1),
    2584, NULL, NULL),    /* heap_overlay_pool */
  };
ai_buffer g_lstm_data_map_weights[AI_LSTM_DATA_WEIGHTS_COUNT] = {
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 90884, 1, 1),
    90884, NULL, s_lstm_weights_array_u64),   /* weights_array */
  };
...
ai_buffer ai_lstm_data_activations_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_LSTM_DATA_ACTIVATIONS_SIZE, 1, AI_LSTM_DATA_ACTIVATIONS_COUNT),
    AI_LSTM_DATA_ACTIVATIONS_SIZE,
    NULL, ptr);
  return buf;
}
...
ai_buffer ai_lstm_data_weights_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_LSTM_DATA_WEIGHTS_SIZE, 1, AI_LSTM_DATA_WEIGHTS_COUNT),
    AI_LSTM_DATA_WEIGHTS_SIZE,
    NULL, ptr);
  return buf;
}
...
{
  return AI_HANDLE_PTR(g_lstm_weights_table);
