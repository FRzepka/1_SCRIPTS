...
#include "lstm_model.h" // For stack measurement
...
int main(void)
{
  ...
  *GPIOE_BSRR = (1UL << (1+16));                    // PE1 OFF
  ...
  while (1)
  {
      ...
      if (c=='\r' || c=='\n') {
        if (rxi > 0) {
          ...
          if (ok) {
            ...
            // BASE model activity on LD1 (PB0)
            ...
            if (rc == 0) {
              ...
              // Output: SOC only (no V*3 debug output for speed)
    ...
    static uint32_t last=0; if (HAL_GetTick() - last > interval) { last = HAL_GetTick(); HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0); }
  }
  /* USER CODE END 3 */
}
