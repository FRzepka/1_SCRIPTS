...
Usage example (Windows cmd):
  ...
  python STM32\bench_host\benchmark_stm32_soc.py --port COM3 --samples 5000
...
def load_test_data(path, samples=None):
    ...
    if ext == '.parquet':
        ...
        # Try to reconstruct DF if possible, otherwise fail
        if 'features' in data and 'soc' in data:
            ...
            raise ValueError(f"NPZ file {path} must contain 'features' and 'soc' arrays.")
    else:
    ...
    col_map = {
        ...
        'Temperature[degC]': ['Temperature[degC]', 'Temperature', 'temp', 'T'],
        'Q_c': ['Q_c', 'capacity', 'Ah'],
        'dU_dt': ['dU_dt', 'dV_dt'],
        'dI_dt': ['dI_dt']
    }
...
def run_benchmark(port, baud, samples, rate_delay, timeout, outdir, seed=None, data_path=None):
    ...
    if data_path and os.path.exists(data_path):
        ...
        if data_path:
            print(f"Warning: Data file {data_path} not found. Using synthetic data.")
    ...
    map_path = os.path.join('..', 'AI_Project_LSTM_SOC_base', 'Debug', 'AI_Project_LSTM_SOC_base.map')
    map_path = os.path.normpath(os.path.join(os.path.dirname(__file__), map_path))
    elf_path = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'AI_Project_LSTM_SOC_base', 'Debug', 'AI_Project_LSTM_SOC_base.elf'))
...
| **Device Inference Time** | {stats['metrics_us_mean']:.1f} | µs |
| **Device Energy (Est.)** | {stats['metrics_E_uJ_mean']:.2f} | µJ |
...
if __name__ == '__main__':
    ...
    ap.add_argument('--outdir', default=None, help='Output directory (defaults to STM32/bench_host/results_TIMESTAMP)')
    ...
    if args.outdir is None:
        args.outdir = os.path.join(os.path.dirname(__file__), 'results_' + datetime.now().strftime('%Y%m%d_%H%M%S'))
