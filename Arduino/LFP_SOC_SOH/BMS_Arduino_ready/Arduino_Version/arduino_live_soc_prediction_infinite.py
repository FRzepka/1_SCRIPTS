...
# suppress warnings for sklearn feature names and missing glyphs
warnings.filterwarnings("ignore", message="X does not have valid feature names", category=UserWarning)
warnings.filterwarnings("ignore", message="Glyph .* missing from font", category=UserWarning)

# Logging Setup mit Emojis
...
class ArduinoLSTMSOCPredictor:
    ...
    def __init__(self, 
        ...
        self.figure = None
        self.axes = None
        self.ax_vc_twin = None  # twin axis for voltage/current
    ...
    def scale_features(self, voltage, current, soh, q_c):
        ...
        try:
            # assemble DataFrame with original feature names to match scaler
            feature_names = getattr(self.scaler, 'feature_names_in_', ['Voltage[V]','Current[A]','SOH_ZHU','Q_c'])
            df_feat = pd.DataFrame([[voltage, current, soh, q_c]], columns=feature_names)
            scaled = self.scaler.transform(df_feat)
            return scaled[0]
        except Exception as e:
            logger.warning(f"⚠️ Skalierung fehlgeschlagen: {e}")
    ...
    def setup_plots(self):
        ...
        # create twin axis once to avoid duplicates
        self.ax_vc_twin = self.axes[1, 0].twinx()
    ...
    def update_plots(self):
        ...
        if len(self.predictions) < 2:
            return
        try:
            # Clear all axes including twin
            for ax in list(self.axes.flat) + [self.ax_vc_twin]:
                ax.clear()
                ax.grid(True, alpha=0.3)
            
            # prepare data, limit to last N points to speed up
            max_display = 200
            length = len(self.predictions)
            start = max(0, length - max_display)
            samples = list(range(start, length))
            preds = list(self.predictions)[start:]
            truths = list(self.ground_truths)[start:]
            errors = list(self.errors)[start:]
            voltages = list(self.voltages)[start:]
            currents = list(self.currents)[start:]
            comm_times = [t * 1000 for t in list(self.communication_times)[start:]]
