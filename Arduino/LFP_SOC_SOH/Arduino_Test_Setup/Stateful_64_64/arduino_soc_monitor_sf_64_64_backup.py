...
class ArduinoSOCMonitor64:
    ...
    def connect_arduino(self):
        """Simplified Arduino connection, less strict on initial READY signal."""
        ...
        try:
            ...
            if self.arduino and self.arduino.is_open:
                self.arduino.close() # Close previous connection if any
            
            self.arduino = serial.Serial(self.port, self.baudrate, timeout=1) # timeout for read operations
            logger.info("Serial port opened. Giving Arduino ~5 seconds to initialize...")
            # This sleep allows Arduino to run its setup() after the serial port is opened by Python.
            # The Arduino sketch's setup() includes while(!Serial); which waits for this.
            time.sleep(5) 
            
            if self.arduino.is_open:
                self.arduino.flushInput() # Clear any stale data from buffer before reader thread starts
                self.arduino.flushOutput()
                logger.info("✅ Connection to Arduino established (or assumed). Reader thread will handle incoming data.")
                self.arduino_connected = True
                return True
            else:
                # This case should ideally not be reached if serial.Serial() succeeded
                logger.error("Serial port not open after attempting connection.")
                self.arduino_connected = False
                return False
            ...
            logger.error(f"An unexpected error occurred during Arduino connection: {e_conn}")
            self.arduino = None
            self.arduino_connected = False
            return False
            ...
            try:
                if self.arduino.in_waiting > 0:
                    ...
                    if line:
                        ...
                        elif "READY" in line or "Arduino LSTM SOC Predictor" in line or "Model:" in line: 
                            logger.info(f"Arduino says: {line}")
                        elif "Error:" in line or "CRITICAL:" in line or "WARNING:" in line : 
    ...
    def start_monitoring(self, auto_upload=True):
        ...
        if auto_upload:
            if not self.upload_arduino_sketch():
                logger.warning("Continuing without successful sketch upload.")
                # Decide if to proceed: for now, we do.
        
        if not self.load_ground_truth_data():
            logger.error("Failed to load ground truth data. Monitoring cannot start effectively.")
            # return # Or allow to run without ground truth for just prediction viewing

        if not self.connect_arduino(): # This now waits for READY
            logger.error("Failed to connect to Arduino. Please check port and connection.")
            return # Cannot run without Arduino

        self.prediction_running = True
        self.start_time = time.time()

        # Explicitly request initial stats after connection
        logger.info("Requesting initial RAM and STATS from Arduino...")
        if not self.send_to_arduino("RAM\\n"):
            self.communication_errors += 1
        ...
        self.reader_thread = threading.Thread(target=self._read_arduino_output_thread, daemon=True)
        self.reader_thread.start()
        
        # Start the background prediction loop (sends data and requests stats)
        ...
        try:
            ani = FuncAnimation(self.fig, self.update_plots, interval=UPDATE_INTERVAL, cache_frame_data=False, blit=False) # blit=False for text
            plt.show()
        except Exception as e:
