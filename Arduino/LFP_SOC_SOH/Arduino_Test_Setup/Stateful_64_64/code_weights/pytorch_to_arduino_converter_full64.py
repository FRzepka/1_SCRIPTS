"""
PyTorch zu Arduino Konverter - VOLLSTÄNDIGE 64 Hidden Units
Extrahiert die exakten Gewichte aus best_model.pth für Arduino UNO R4
KEINE Komprimierung - vollständige 64x64 Modell-Architektur!
"""
...
NUM_LAYERS = 1
MLP_HIDDEN = 64
MODEL_PATH = r"c:\Users\Florian\SynologyDrive\TUB\1_Dissertation\5_Codes\LFP_SOC_SOH\Arduino_Test_Setup\Stateful_64_64\model\best_model.pth"
...
def analyze_exact_weights(model):
    ...
    print("\n🔍 VOLLSTÄNDIGE GEWICHTS-ANALYSE (64 Hidden Units):")
    
    # LSTM Gewichte
    lstm_weight_ih = model.lstm.weight_ih_l0.data.numpy()  # [256, 4]
    lstm_weight_hh = model.lstm.weight_hh_l0.data.numpy()  # [256, 64]
    lstm_bias_ih = model.lstm.bias_ih_l0.data.numpy()      # [256]
    lstm_bias_hh = model.lstm.bias_hh_l0.data.numpy()      # [256]
    ...
    # MLP Gewichte
    mlp0_weight = model.mlp[0].weight.data.numpy()  # [64, 64]
    mlp0_bias = model.mlp[0].bias.data.numpy()      # [64]
    mlp3_weight = model.mlp[3].weight.data.numpy()  # [64, 64]
    mlp3_bias = model.mlp[3].bias.data.numpy()      # [64]
    mlp6_weight = model.mlp[6].weight.data.numpy()  # [1, 64]
...
def create_arduino_weights_full64(weights):
    """Erstelle Arduino-Gewichte für vollständige 64-Hidden-Architecture"""
    
    print(f"\n⚙️ ARDUINO-GEWICHTE ERSTELLEN (VOLLSTÄNDIGE 64 HIDDEN UNITS):")
    
    # LSTM Gewichte komplett verwenden - KEINE Reduktion!
    # PyTorch LSTM Layout: [4*hidden_size, input_size] = [256, 4]
    # Gates: input, forget, candidate, output (je 64 Neuronen)
    
    arduino_weights = {}
    
    # Für jedes Gate ALLE 64 Neuronen verwenden
    for gate_idx, gate_name in enumerate(['input', 'forget', 'candidate', 'output']):
        ...
        gate_ih = weights['lstm_weight_ih'][gate_start:gate_end, :]  # [64, 4]
        arduino_weights[f'lstm_{gate_name}_ih_weights'] = gate_ih
        
        # Hidden-zu-Hidden Gewichte für dieses Gate - ALLE verwenden!
        gate_hh = weights['lstm_weight_hh'][gate_start:gate_end, :]  # [64, 64]
        arduino_weights[f'lstm_{gate_name}_hh_weights'] = gate_hh
        
        # Bias für dieses Gate - separat wie PyTorch!
        gate_bias_ih = weights['lstm_bias_ih'][gate_start:gate_end]  # [64]
        gate_bias_hh = weights['lstm_bias_hh'][gate_start:gate_end]  # [64]
    ...
    arduino_weights['mlp0_weights'] = weights['mlp0_weight']  # [64, 64]
    arduino_weights['mlp0_bias'] = weights['mlp0_bias']      # [64]
    
    arduino_weights['mlp3_weights'] = weights['mlp3_weight']  # [64, 64]
    arduino_weights['mlp3_bias'] = weights['mlp3_bias']      # [64]
    
    arduino_weights['mlp6_weights'] = weights['mlp6_weight']  # [1, 64]
...
def generate_arduino_header_full64(arduino_weights, output_file):
    """Generiere Arduino Header mit vollständigen 64-Hidden-Unit Gewichten"""
    
    total_params = 0
    
    header_content = '''/*
 * VOLLSTÄNDIGE LSTM Gewichte aus trainiertem PyTorch Modell
 * Modell: BMS_SOC_LSTM_stateful_1.2.4 (64x64)
 * Arduino Hidden Size: 64 (VOLLSTÄNDIGE ARCHITEKTUR)
 * Exakte 1:1 Konvertierung - KEINE Komprimierung!
 * Ziel: Arduino UNO R4 (32KB SRAM) - WARNUNG: Speicherverbrauch hoch!
 */

#ifndef LSTM_WEIGHTS_FULL64_H
#define LSTM_WEIGHTS_FULL64_H

// Arduino LSTM Konfiguration
#define ARDUINO_HIDDEN_SIZE 64
#define INPUT_SIZE 4
...
'''
    ...
    if memory_percent > 100:
        print("🚨 KRITISCH: Speicherverbrauch > 100% - wird NICHT auf Arduino UNO R4 laufen!")
        print("💡 Empfehlung: Arduino Mega 2560 (8KB SRAM) oder ESP32 verwenden")
...
def main():
    """Hauptfunktion für vollständige 64-Hidden-Unit Konvertierung"""
    print("🎯 VOLLSTÄNDIGER PyTorch zu Arduino Konverter (64 Hidden Units)")
    print("=" * 60)
    ...
    arduino_weights = create_arduino_weights_full64(weights)
    
    # 4. Arduino Header generieren
    output_file = "lstm_weights.h"
    total_params = generate_arduino_header_full64(arduino_weights, output_file)
    
    print(f"\n🎉 VOLLSTÄNDIGE KONVERTIERUNG ABGESCHLOSSEN!")
    print(f"📁 Output: {output_file}")
    print(f"🔧 Arduino Hidden Size: 64 (vollständig)")
