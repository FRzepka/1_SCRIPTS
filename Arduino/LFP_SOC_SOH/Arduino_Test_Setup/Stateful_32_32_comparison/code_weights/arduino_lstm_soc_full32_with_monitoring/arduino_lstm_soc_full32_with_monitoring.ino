...
// NICHT die Compiler-Schätzungen der statischen Variablen!

int getFreeRam() {
  // ARM Cortex-M basierte dynamische RAM-Messung
  // Stack-Pointer minus Heap-Ende = verfügbarer RAM
...
// ============ ENHANCED RAM DIAGNOSTICS (Arduino UNO R4 Compatible) ============
void printDetailedRamInfo() {
  // Arduino UNO R4 WiFi kompatible RAM-Analyse - ECHTE MESSWERTE, KEINE SCHÄTZUNGEN!
  Serial.println("RAM_DEBUG:DETAILED_ANALYSIS");
  
  // Echte RAM-Messwerte vom Arduino UNO R4 WiFi (Renesas RA4M1)
  int total_ram = getTotalRam();
  int free_ram = getFreeRam();
  int used_ram = getUsedRam();
  float usage_percent = getRamFragmentation();
  
  Serial.print("RAM_DEBUG:Total_RAM="); Serial.print(total_ram); Serial.println("B");
  Serial.print("RAM_DEBUG:Free_RAM="); Serial.print(free_ram); Serial.println("B");
  Serial.print("RAM_DEBUG:Used_RAM="); Serial.print(used_ram); Serial.println("B");
  Serial.print("RAM_DEBUG:Usage_Percent="); Serial.print(usage_percent, 2); Serial.println("%");
  
  // Heap-Pointer (Arduino-kompatibel)
  char *heap_ptr = sbrk(0);
  Serial.print("RAM_DEBUG:Heap_Pointer=0x"); Serial.println((uint32_t)heap_ptr, HEX);
  
  // Stack-basierte Messung (Arduino-sicher)
  volatile uint8_t stack_var = 42;
  Serial.print("RAM_DEBUG:Stack_Variable=0x"); Serial.println((uint32_t)&stack_var, HEX);
  
  // Speicherabstand zwischen Stack und Heap
  uint32_t heap_stack_gap = (uint32_t)&stack_var - (uint32_t)heap_ptr;
  Serial.print("RAM_DEBUG:Heap_Stack_Gap="); Serial.print(heap_stack_gap); Serial.println("B");
  
  Serial.println("RAM_DEBUG:PROOF_REAL_TIME_MEASUREMENT");
  Serial.print("RAM_DEBUG:Timestamp="); Serial.println(millis());
}

// Flash Memory Functions - Arduino UNO R4 WiFi (RA4M1) with Real Measurements
int getTotalFlash() {
  return flashend - flashstart;  // 256KB Flash total
}

int getUsedFlash() {
  // Use linker symbols to get real Flash usage for Arduino UNO R4
  #if defined(ARDUINO_UNOR4_WIFI) || defined(ARDUINO_UNOR4_MINIMA)
    // Real Flash usage from linker symbols - Arduino UNO R4 WiFi (RA4M1)
    // Using symbols defined in fsp.ld linker script
    extern char __etext[];        // End of text (code) section
    extern char __data_start__[]; // Start of initialized data in RAM
    extern char __data_end__[];   // End of initialized data in RAM
    
    // Flash starts at 0x00000000
    uint32_t flash_start = 0x00000000;
    uint32_t code_size = (uint32_t)__etext - flash_start;
    uint32_t data_size = (uint32_t)__data_end__ - (uint32_t)__data_start__; // Data copy in Flash
    uint32_t total_used_flash = code_size + data_size;
    return (int)total_used_flash;
  #else
    // Fallback estimates for other Arduino boards
    int total_flash = getTotalFlash();
    
    if (total_flash <= 32768) {
      // Arduino Uno/Nano (32KB Flash) - conservative estimate
      return 28 * 1024; // 28KB used
    } else if (total_flash <= 262144) {
      // Arduino Mega (256KB Flash) - larger program estimate
      return 180 * 1024; // 180KB used
    } else {
      // Larger boards (Due, ESP32, etc.) - scale with available flash
      return total_flash / 4; // Use 25% as estimate
    }
  #endif
}
...
void printBenchmarkResults() {
  ...
  // Force Arduino UNO R4 WiFi detection by sending 48MHz
  #if defined(ARDUINO_UNOR4_WIFI) || defined(ARDUINO_UNOR4_MINIMA)
    Serial.print(48); // Arduino UNO R4 WiFi/Minima: 48 MHz
  #else
    Serial.print(F_CPU / 1000000); // Other boards: use actual frequency
  #endif
...
void performBenchmark() {
  ...
  // DETAILLIERTE RAM-ANALYSE AUSGEBEN
  Serial.println("=== DETAILED RAM ANALYSIS (REAL MEASUREMENTS) ===");
  printDetailedRamInfo();
  Serial.println("=== END RAM ANALYSIS ===");
