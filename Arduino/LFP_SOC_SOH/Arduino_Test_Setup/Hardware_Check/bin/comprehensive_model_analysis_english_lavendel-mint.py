...
class ModelArchitectureAnalyzer:
    def __init__(self, base_path):
        ...
        # Elegant mint-green and lavender color scheme inspired by your diagram
        self.color_scheme = {
            # Main colors - soft mint greens and lavender
            'stateful_lstm': '#A8E6CF',      # Soft mint green for Stateful LSTM
            'windows_lstm': '#D4A5D4',       # Soft lavender for Windows LSTM  
            'stateful_secondary': '#C8F0DB', # Very light mint for secondary elements
            'windows_secondary': '#E6D0E6',  # Very light lavender for secondary elements
            
            # Architecture-specific colors - unified elegant approach
            'stateful_16': '#A8E6CF',        # Base soft mint green
            'stateful_32': '#A8E6CF',        # Same mint, consistency
            'stateful_64': '#A8E6CF',        # Same mint, consistency
            'stateful_16_secondary': '#C8F0DB',  # Light mint
            'stateful_32_secondary': '#C8F0DB',  # Same light mint
            'stateful_64_secondary': '#C8F0DB',  # Same light mint
            
            'windows_16': '#D4A5D4',         # Base soft lavender
            'windows_32': '#D4A5D4',         # Same lavender, consistency
            'windows_64': '#D4A5D4',         # Same lavender, consistency
            'windows_16_secondary': '#E6D0E6',   # Light lavender
            'windows_32_secondary': '#E6D0E6',   # Same light lavender
            'windows_64_secondary': '#E6D0E6'    # Same light lavender
        }
    ...
    def create_comprehensive_visualization(self):
        ...
        for i, (status, (primary_color, secondary_color)) in enumerate(zip(validation_status, model_colors)):
            if status == 'match':
                ...
                scatter_colors.append('red')  # Red for errors/unknown        # Scatter plots with model-specific colors
        ...
        scatter2 = ax2.scatter(model_params, theoretical_file_sizes, c='#B8A9D4', s=80, alpha=0.6,
                              label='Theoretical File Size (4 bytes/param)', marker='^', edgecolors='black')        # Connection lines - softer color from your diagram
        for i in range(len(model_params)):
            if model_params[i] > 0:
                ax2.plot([model_params[i], model_params[i]], 
                        [theoretical_file_sizes[i], file_sizes[i]], 
                        '--', color='#E6D0E6', alpha=0.4, linewidth=1)  # Light lavender
        ...
        if hidden_sizes and any(h > 0 for h in hidden_sizes):
            ...
            ax3.plot(theoretical_curve_x, theoretical_curve_y, '-', color='#D4A5D4', linewidth=2,
                    label='Theoretical Curve (LSTM + MLP)')  # Elegant lavender
        ...
        ax4.text(0.05, 0.95, summary_text, transform=ax4.transAxes, 
                ...
                bbox=dict(boxstyle="round,pad=0.5", facecolor="#E6D0E6", alpha=0.3))  # Light lavender background
        ax4.set_xlim(0, 1)
        ax4.set_ylim(0, 1)
        ax4.axis('off')
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        flash_colors = ["#C8F0DB", '#C8F0DB', '#C8F0DB']  # Light mint for Flash (from your diagram)
        ram_colors = ['#E6D0E6', '#E6D0E6', '#E6D0E6']    # Light lavender for RAM (from your diagram)
        ...
        bars1 = ax1.bar(x - width*1.5, flash_est, width, label='Flash Estimation', 
                       alpha=0.8, color=flash_bar_colors, edgecolor='#7DB8A3', linewidth=1)  # Mint edge
        ...
        if flash_real_filtered:
            ...
            flash_real_bar_colors = ['#A8E6CF'] * len(flash_real_filtered)  # Your soft mint
            bars2 = ax1.bar(x[:len(flash_real_filtered)] - width*0.5, flash_real_filtered, width, 
                           ...
                           edgecolor='#7DB8A3', linewidth=1)  # Darker mint edge
          # RAM bars  
        bars3 = ax1.bar(x + width*0.5, ram_est, width, label='RAM Estimation', 
                       alpha=0.8, color=ram_bar_colors, edgecolor='#C8A8C8', linewidth=1)  # Lavender edge
        ...
        if ram_real_filtered:
            ...
            ram_real_bar_colors = ['#D4A5D4'] * len(ram_real_filtered)  # Your soft lavender
            bars4 = ax1.bar(x[:len(ram_real_filtered)] + width*1.5, ram_real_filtered, width,
                           ...
                           edgecolor='#C8A8C8', linewidth=1)  # Darker lavender edge        # Arduino limits - use elegant soft colors from your diagram
        ax1.axhline(y=256, color='#A8E6CF', linestyle='--', alpha=0.7, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax1.axhline(y=32, color='#D4A5D4', linestyle='--', alpha=0.7,
        ...
        flash_color = '#A8E6CF'  # Soft mint green for Flash (like your diagram)
        ram_color = '#D4A5D4'    # Soft lavender for RAM (accent color from your diagram)
        ...
        real_flash_color = '#C8F0DB'  # Very light mint for real Flash (from your diagram)
        real_ram_color = '#B8A9D4'   # Medium lavender for real RAM (from your diagram)
        ...
        ax2.axhline(y=256, color='#A8E6CF', linestyle='--', alpha=0.8, linewidth=3, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax2.axhline(y=32, color='#D4A5D4', linestyle='--', alpha=0.8, linewidth=3,
                   label='Arduino R4 RAM Limit (32 kB)')
        ...
        if estimation_data:
            ...
            colors_pie = ['#A8E6CF', '#C8F0DB']  # Your mint green palette
        ...
        if estimation_data:
            ...
            colors_pie = ['#D4A5D4', '#E6D0E6']  # Your lavender palette
