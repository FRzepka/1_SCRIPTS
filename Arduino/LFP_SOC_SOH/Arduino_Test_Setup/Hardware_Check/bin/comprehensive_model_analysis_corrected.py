...
class ModelArchitectureAnalyzer:
    ...
    def calculate_arduino_flash_ram_estimation(self, hidden_size, input_size=1):
        ...
        total_parameters = model_params['total']
          # Flash-Abschätzung - also simplified to English categories  
        ...
        estimated_flash_kb = sum(flash_components.values())
          # RAM-Abschätzung - simplified categories
    ...
    def create_comprehensive_visualization(self):
        ...
        for theoretical, measured in zip(script_params, model_params):
            if theoretical > 0 and measured > 0:
                ...
                errors.append(0)        # Balken mit Fehlerbalken
        bars1 = ax1.bar(x - width/2, script_params, width, label='Formula (LSTM+MLP)', alpha=0.7, color='lightblue')
        bars2 = ax1.bar(x + width/2, model_params, width, label='PyTorch Model (Actual)', alpha=0.7, color='orange')
        ...
        ax1.set_title('Parameter Comparison: Theoretical Formula vs. PyTorch Model (with Error Bars)')
        ax1.set_xticks(x)
        ax1.set_xticklabels(architectures, rotation=45)
        ax1.legend()
        ax1.grid(True, alpha=0.3)        # Werte auf Balken
        ...
        from matplotlib.lines import Line2D        legend_elements = [
            Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=8, alpha=0.7, label='Measured File Size'),
            Line2D([0], [0], marker='^', color='w', markerfacecolor='blue', markersize=8, alpha=0.5, label='Theoretical File Size (4 bytes/param)'),
            Patch(facecolor='green', alpha=0.7, label='Formula ↔ Model Match'),
            Patch(facecolor='orange', alpha=0.7, label='Formula ↔ Model Mismatch'),
            Patch(facecolor='red', alpha=0.7, label='Unknown/Error')
        ]
        ...
        if hidden_sizes and any(h > 0 for h in hidden_sizes):
            ...
            # Measured models - use the correct colors
            ...
            ax3.scatter(valid_hidden, valid_params, c=valid_colors, s=100, alpha=0.8, 
                       marker='o', label='Measured Models', zorder=5)
        ...
        else:            ax3.text(0.5, 0.5, 'No Hidden Size\nData Available', 
                    ha='center', va='center', transform=ax3.transAxes, fontsize=14)        # ========== 4. Summary ==========
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        ax1.legend()
        ax1.grid(True, alpha=0.3)        # ========== 2. Logarithmic Scaling with CONTINUOUS ERROR CURVES ==========        # Future prediction: 16, 32, 64, 128, 256, 512
        ...
        ram_lower = [est * (1 - avg_ram_error_pct) for est in log_ram_est]        # MAIN CURVES (Average estimation)
        ...
        # FLASH: Shaded area + error bars (large uncertainty 33.9%)
        ax2.fill_between(future_hidden_sizes, flash_lower, flash_upper, 
                        color='red', alpha=0.2, label='Flash Uncertainty Range')
        ...
        # RAM: ONLY error bars (small uncertainty 3.2%, no shaded area)
        ...
        # Real measurements as reference (large black squares)
        ...
        ax2.scatter(real_hs, real_ram_vals, color='darkblue', s=200, marker='s',
          ...
          # Arduino Limits as horizontal lines
