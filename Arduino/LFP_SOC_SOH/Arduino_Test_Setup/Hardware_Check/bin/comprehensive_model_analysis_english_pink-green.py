...
class ModelArchitectureAnalyzer:
    def __init__(self, base_path):
        ...
        self.results = {}        # Harmonious color scheme - softer, less garish colors based on your palette
        self.color_scheme = {
            # Main colors - softer versions of your palette
            'stateful_lstm': '#FF8888',      # Soft red for Stateful LSTM (from your palette)
            'windows_lstm': '#95E1DB',       # Soft teal for Windows LSTM (from your palette)
            'stateful_secondary': '#FFA6A6', # Lighter red for secondary elements (from your palette)
            'windows_secondary': '#71D7CF',  # Darker teal for secondary elements (from your palette)
            
            # Architecture-specific colors - unified approach
            'stateful_16': '#FF8888',        # Base soft red
            'stateful_32': '#FF8888',        # Same red, consistency
            'stateful_64': '#FF8888',        # Same red, consistency
            'stateful_16_secondary': '#FFA6A6',  # Lighter red
            'stateful_32_secondary': '#FFA6A6',  # Same lighter red
            'stateful_64_secondary': '#FFA6A6',  # Same lighter red
            
            'windows_16': '#95E1DB',         # Base soft teal
            'windows_32': '#95E1DB',         # Same teal, consistency
            'windows_64': '#95E1DB',         # Same teal, consistency
            'windows_16_secondary': '#71D7CF',   # Darker teal
            'windows_32_secondary': '#71D7CF',   # Same darker teal
            'windows_64_secondary': '#71D7CF'    # Same darker teal
        }
    ...
    def create_comprehensive_visualization(self):
        ...
        scatter2 = ax2.scatter(model_params, theoretical_file_sizes, c='#71D7CF', s=80, alpha=0.6,
                              label='Theoretical File Size (4 bytes/param)', marker='^', edgecolors='black')
          # Connection lines - softer color
        for i in range(len(model_params)):
            if model_params[i] > 0:
                ax2.plot([model_params[i], model_params[i]], 
                        [theoretical_file_sizes[i], file_sizes[i]], 
                        '--', color='#95E1DB', alpha=0.4, linewidth=1)
        
        # Annotations
        for i, arch in enumerate(architectures):
            if model_params[i] > 0:
                # Calculate overhead percent
        ...
        ax2.grid(True, alpha=0.3)        # Legend with color coding information - using your harmonious palette
        from matplotlib.patches import Patch
        from matplotlib.lines import Line2D
        legend_elements = [
            Line2D([0], [0], marker='o', color='w', markerfacecolor='#71D7CF', markersize=8, alpha=0.7, label='Measured File Size'),
            Line2D([0], [0], marker='^', color='w', markerfacecolor='#71D7CF', markersize=8, alpha=0.6, label='Theoretical File Size (4 bytes/param)'),
        ...
        if hidden_sizes and any(h > 0 for h in hidden_sizes):
            ...
            ax3.plot(theoretical_curve_x, theoretical_curve_y, '-', color='#71D7CF', linewidth=2,
                    label='Theoretical Curve (LSTM + MLP)')
        ...
        ax4.text(0.05, 0.95, summary_text, transform=ax4.transAxes, 
                ...
                bbox=dict(boxstyle="round,pad=0.5", facecolor="#95E1DB", alpha=0.3))
        ax4.set_xlim(0, 1)
        ax4.set_ylim(0, 1)
        ax4.axis('off')
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        x = np.arange(len(architectures))
        width = 0.2        # Use harmonious color schemes for Flash and RAM bars - your palette
        flash_colors = ["#FFA6A6", '#FFA6A6', '#FFA6A6']  # Light red for Flash (from your palette)
        ram_colors = ['#95E1DB', '#95E1DB', '#95E1DB']    # Soft teal for RAM (from your palette)
        ...
        bars1 = ax1.bar(x - width*1.5, flash_est, width, label='Flash Estimation', 
                       alpha=0.8, color=flash_bar_colors, edgecolor='darkred', linewidth=1)
        flash_real_filtered = [f for f in flash_real if f is not None]
        if flash_real_filtered:
            # Use slightly darker version for real measurements - your palette
            flash_real_bar_colors = ['#FF8888'] * len(flash_real_filtered)  # Your soft red
        ...
        bars3 = ax1.bar(x + width*0.5, ram_est, width, label='RAM Estimation', 
                       alpha=0.8, color=ram_bar_colors, edgecolor='darkblue', linewidth=1)
        ram_real_filtered = [r for r in ram_real if r is not None]
        if ram_real_filtered:
            # Use your palette for real RAM measurements
            ram_real_bar_colors = ['#71D7CF'] * len(ram_real_filtered)  # Your darker teal
        ...
        # Arduino limits - use softer colors
        ax1.axhline(y=256, color='#FF8888', linestyle='--', alpha=0.7, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax1.axhline(y=32, color='#71D7CF', linestyle='--', alpha=0.7,
        ...
        ram_lower = [est * (1 - avg_ram_error_pct) for est in log_ram_est]        # MAIN CURVES (Average estimation) - use harmonious colors from your palette
        flash_color = '#FF8888'  # Soft red for Flash (from your palette)
        ram_color = '#95E1DB'    # Soft teal for RAM (from your palette)
        ...
        # Use your palette colors for real measurements
        real_flash_color = '#FFA6A6'  # Light red for real Flash (from your palette)
        real_ram_color = '#71D7CF'   # Darker teal for real RAM (from your palette)
        ...
        # Arduino limits as horizontal lines - softer colors
        ax2.axhline(y=256, color='#FF8888', linestyle='--', alpha=0.8, linewidth=3, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax2.axhline(y=32, color='#71D7CF', linestyle='--', alpha=0.8, linewidth=3,
                   label='Arduino R4 RAM Limit (32 kB)')
        ...
        if estimation_data:
            ...
            labels = [label.replace('_', ' ').title() for label in flash_components.keys()]
              # Use soft colors from your palette for Flash
            colors_pie = ['#FF8888', '#FFA6A6']  # Your soft red palette
        ...
        if estimation_data:
            ...
            labels = [label.replace('_', ' ').title() for label in ram_components.keys()]
              # Use soft colors from your palette for RAM
            colors_pie = ['#95E1DB', '#71D7CF']  # Your soft teal palette
