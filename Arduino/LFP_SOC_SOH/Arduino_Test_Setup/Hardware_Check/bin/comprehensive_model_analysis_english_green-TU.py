...
class ModelArchitectureAnalyzer:
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        x = np.arange(len(architectures))
        width = 0.2        # Color families for better consistency - light and dark variants
        # Flash uses green family: light green for estimation, dark green for real
        flash_estimation_color = '#b2e1c1'  # Light green (consistent)
        flash_real_color = '#7fb88a'        # Dark green (family variant)
        
        # RAM uses blue family: light blue for estimation, dark blue for real  
        ram_estimation_color = '#c7dfed'    # Light blue (consistent)
        ram_real_color = '#8bb3d1'          # Dark blue (family variant)
          # Flash bars - consistent light green for estimation
        ...
        if flash_real_filtered:            # Dark green for real flash measurements
        ...
        # RAM bars - consistent light blue for estimation  
        ...
        if ram_real_filtered:            # Dark blue for real RAM measurements
            bars4 = ax1.bar(x[:len(ram_real_filtered)] + width*1.5, ram_real_filtered, width,
                           label='RAM Real', alpha=0.9, color=ram_real_color, 
                           edgecolor='black', linewidth=0.5)
        
        # Arduino limits - kräftige Akzentfarben
        ax1.axhline(y=256, color='#2091C9', linestyle='--', alpha=0.7, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax1.axhline(y=32, color='#2091C9', linestyle='--', alpha=0.7,
        ...
        ram_lower = [est * (1 - avg_ram_error_pct) for est in log_ram_est]        # MAIN CURVES - lighter colors for better visibility
        flash_color = '#4a9d6b'  # Lighter green for Flash curves (was #2e7d5b)
        ram_color = '#2e6bb8'    # Lighter blue for RAM curves (was #1e5a96)
        ...
        # Real measurements as reference - use family-consistent colors
        real_hs = list(real_measurements.keys())
        real_flash_vals = [real_measurements[hs]['flash_used_kb'] for hs in real_hs]
        real_ram_vals = [real_measurements[hs]['ram_used_kb'] for hs in real_hs]        # Real measurements in light pastel colors for contrast with dark curves
        real_flash_color = '#b2e1c1'  # Light green for real Flash (same as bar)
        real_ram_color = '#c7dfed'    # Light blue for real RAM (same as bar)
        ...
        # Arduino limits - lighter accent color for better contrast with dark curves
        ax2.axhline(y=256, color='#64B5F6', linestyle='--', alpha=0.6, linewidth=2, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax2.axhline(y=32, color='#64B5F6', linestyle='--', alpha=0.6, linewidth=2,
                   label='Arduino R4 RAM Limit (32 kB)')
        ...
        if estimation_data:
            ...
            labels = [label.replace('_', ' ').title() for label in flash_components.keys()]            # Elegante Pastellfarben für Flash Pie Chart
            colors_pie = ['#b2e1c1', '#cde8e1']  # Helles Grün + Sanftes Mint-Grün
        ...
        if estimation_data:
            ...
            labels = [label.replace('_', ' ').title() for label in ram_components.keys()]            # Elegante Pastellfarben für RAM Pie Chart
            colors_pie = ['#c7dfed', '#afd9dc']  # Helles Blau + Sanftes Türkis
