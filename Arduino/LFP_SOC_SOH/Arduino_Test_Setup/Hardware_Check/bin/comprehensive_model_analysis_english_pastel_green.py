...
class ModelArchitectureAnalyzer:
    def __init__(self, base_path):
        ...
        self.results = {}        # Professionelles Farbschema - Helle Pastelltöne + kräftige Akzente
        self.color_scheme = {
            # 🎨 Main Stateful LSTM colors - helle Grün-Töne (Pastellfarben)
            'stateful_lstm': '#b2e1c1',      # Helles Grün (Hauptfarbe für Stateful LSTM)
            'stateful_secondary': '#cde8e1', # Sanftes Mint-Grün (sekundäre Elemente)
            
            # 🔵 Main Windows LSTM colors - helle Blau-Töne (Pastellfarben)
            'windows_lstm': '#c7dfed',       # Helles Blau (Hauptfarbe für Windows LSTM)
            'windows_secondary': '#afd9dc',  # Sanftes Türkis (sekundäre Elemente)
            
            # Architecture-specific colors - Grün-Varianten für Stateful (Pastellfarben)
            'stateful_16': '#b2e1c1',        # Helles Grün (16x16)
            'stateful_32': '#cde8e1',        # Sanftes Mint-Grün (32x32) 
            'stateful_64': '#b2e1c1',        # Helles Grün (64x64)
            'stateful_16_secondary': '#cde8e1',  # Sanftes Mint-Grün (sekundär)
            'stateful_32_secondary': '#b2e1c1',  # Helles Grün (sekundär)
            'stateful_64_secondary': '#cde8e1',  # Sanftes Mint-Grün (sekundär)
            
            # Architecture-specific colors - Blau-Varianten für Windows (Pastellfarben)
            'windows_16': '#c7dfed',         # Helles Blau (16x16)
            'windows_32': '#afd9dc',         # Sanftes Türkis (32x32)
            'windows_64': '#c7dfed',         # Helles Blau (64x64)
            'windows_16_secondary': '#afd9dc',   # Sanftes Türkis (sekundär)
            'windows_32_secondary': '#afd9dc',   # Sanftes Türkis (sekundär)
            'windows_64_secondary': '#afd9dc',   # Sanftes Türkis (sekundär)
    ...
    def create_comprehensive_visualization(self):
        ...
        for theoretical, measured in zip(script_params, model_params):
            if theoretical > 0 and measured > 0:
                ...
                errors.append(0)        # Simplified color scheme - only 2 green tones for consistency
        # Light green for Formula, Dark green for Actual (consistent across all architectures)
        formula_color = '#cde8e1'    # Light green for all Formula bars
        actual_color = '#afd9dc'     # Dark green for all Actual bars
        
        bars1 = ax1.bar(x - width/2, script_params, width, label='Formula (LSTM+MLP)', 
                       alpha=0.8, color=formula_color, edgecolor='black', linewidth=0.5)
        bars2 = ax1.bar(x + width/2, model_params, width, label='PyTorch Model (Actual)', 
                       alpha=0.9, color=actual_color, edgecolor='black', linewidth=0.5)
        ...
        ax1.set_xticks(x)
        ax1.set_xticklabels(architectures, rotation=45)        # Simplified legend for parameter comparison - only 2 colors
        from matplotlib.patches import Patch
        legend_elements = [
            Patch(facecolor=formula_color, alpha=0.8, label='Formula (All Architectures)'),
            Patch(facecolor=actual_color, alpha=0.9, label='Actual (All Architectures)')
        ]
            ...
            for bar in bars:
                ...
                if height > 0:
                    ax1.text(bar.get_x() + bar.get_width()/2., height + height*0.02,
                            f'{int(height):,}', ha='center', va='bottom', fontweight='bold', fontsize=9)          # ========== 2. File Size vs. Parameter ==========
        # Simplified scatter colors - consistent with simplified color scheme
        scatter_colors = []
        for status in validation_status:
            if status == 'match':
                scatter_colors.append(actual_color)     # Dark green for matches
            elif status == 'mismatch':
                scatter_colors.append(formula_color)    # Light green for mismatches
            else:
                scatter_colors.append(self.color_scheme['error_color'])  # Red for errors/unknown        # Scatter plots with simplified colors
        scatter1 = ax2.scatter(model_params, file_sizes, c=scatter_colors, s=120, alpha=0.8, 
                              label='Measured File Size', marker='o', edgecolors='black', linewidth=0.5)
        scatter2 = ax2.scatter(model_params, theoretical_file_sizes, c='#2091C9', s=80, alpha=0.6,
                              label='Theoretical File Size (4 bytes/param)', marker='^', edgecolors='black', linewidth=0.5)

        # Connection lines - lebendiges Blau (Akzentfarbe)
        for i in range(len(model_params)):
            if model_params[i] > 0:
                ax2.plot([model_params[i], model_params[i]], 
                        [theoretical_file_sizes[i], file_sizes[i]], 
                        '--', color='#2091C9', alpha=0.4, linewidth=1)  # Lebendiges Blau
        ...
        ax2.grid(True, alpha=0.3)        # Simplified legend for scatter plots - consistent with simplified color scheme  
        from matplotlib.lines import Line2D
        legend_elements = [
            Line2D([0], [0], marker='o', color='w', markerfacecolor=actual_color, markersize=8, alpha=0.8, label='Measured File Size'),
            Line2D([0], [0], marker='^', color='w', markerfacecolor='#2091C9', markersize=8, alpha=0.6, label='Theoretical File Size (4 bytes/param)'),
            Patch(facecolor=actual_color, alpha=0.8, label='Model Match/Good'),
            Patch(facecolor=formula_color, alpha=0.7, label='Model Mismatch'),
            Patch(facecolor=self.color_scheme['error_color'], alpha=0.7, label='Unknown/Error')
        ]
        ...
        if hidden_sizes and any(h > 0 for h in hidden_sizes):
            ...
            ax3.plot(theoretical_curve_x, theoretical_curve_y, '-', color=actual_color, linewidth=2,
                    label='Theoretical Curve (LSTM + MLP)')  # Dark green for curve
            
            # Measured models - use consistent dark green color for all models
            ...
            ax3.scatter(valid_hidden, valid_params, c=actual_color, s=120, alpha=0.8, 
                       marker='o', label='Measured Models', zorder=5, edgecolors='black', linewidth=0.5)
        ...
        ax4.text(0.05, 0.95, summary_text, transform=ax4.transAxes, 
                ...
                bbox=dict(boxstyle="round,pad=0.5", facecolor="#c7dfed", alpha=0.3))  # Helles Blau (Pastellfarbe)
        ax4.set_xlim(0, 1)
        ax4.set_ylim(0, 1)
        ax4.axis('off')
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        # Define colors from self.color_scheme for clarity and consistency
        # Flash colors (Green-themed)
        flash_estimation_color = self.color_scheme.get('stateful_secondary', '#cde8e1') # Light green
        flash_real_color = self.color_scheme.get('stateful_lstm', '#b2e1c1')          # Darker green
        
        # RAM colors (Blue-themed)
        ram_estimation_color = self.color_scheme.get('windows_secondary', '#afd9dc')  # Light blue
        ram_real_color = self.color_scheme.get('windows_lstm', '#c7dfed')            # Darker blue
        
        # Accent color for limits
        limit_color = self.color_scheme.get('accent_blue', '#2091C9')
        
        # Flash bars
        bars1 = ax1.bar(x - width*1.5, flash_est, width, label='Flash Estimation', 
                       alpha=0.8, color=flash_estimation_color, edgecolor='black', linewidth=0.5)
        ...
        if flash_real_filtered:
            bars2 = ax1.bar(x[:len(flash_real_filtered)] - width*0.5, flash_real_filtered, width, 
                           label='Flash Real', alpha=0.9, color=flash_real_color, 
                           edgecolor='black', linewidth=0.5)
        
        # RAM bars
        bars3 = ax1.bar(x + width*0.5, ram_est, width, label='RAM Estimation', 
                       alpha=0.8, color=ram_estimation_color, edgecolor='black', linewidth=0.5)
        ...
        if ram_real_filtered:
            bars4 = ax1.bar(x[:len(ram_real_filtered)] + width*1.5, ram_real_filtered, width,
                           label='RAM Real', alpha=0.9, color=ram_real_color, 
                           edgecolor='black', linewidth=0.5)
        
        # Arduino limits
        ax1.axhline(y=256, color=limit_color, linestyle='--', alpha=0.7, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax1.axhline(y=32, color=limit_color, linestyle='--', alpha=0.7,
        ...
        # MAIN CURVES - Use consistent colors from the scheme
        flash_curve_color = self.color_scheme.get('stateful_lstm', '#b2e1c1') # Darker green for Flash curves 
        ram_curve_color = self.color_scheme.get('windows_lstm', '#c7dfed')    # Darker blue for RAM curves
        
        ax2.plot(future_hidden_sizes, log_flash_est, 'o-', color=flash_curve_color, alpha=0.9, 
                label='Flash Estimation', linewidth=3, markersize=10)
        ax2.plot(future_hidden_sizes, log_ram_est, 's-', color=ram_curve_color, alpha=0.9,
        ...
        ax2.fill_between(future_hidden_sizes, flash_lower, flash_upper, 
                        color=flash_curve_color, alpha=0.2, label='Flash Uncertainty Range')
        ...
        ax2.errorbar(future_hidden_sizes, log_flash_est, yerr=flash_errors_absolute,
                    fmt='none', color=flash_curve_color, alpha=0.8, 
        ...
        ax2.errorbar(future_hidden_sizes, log_ram_est, yerr=ram_errors_absolute,
                    fmt='none', color=ram_curve_color, alpha=0.8,
                    capsize=8, capthick=3, linewidth=2, label='RAM Error Bars')
        
        # UPPER AND LOWER BOUNDARY LINES only for Flash (dashed)
        ax2.plot(future_hidden_sizes, flash_upper, '--', color=flash_curve_color, alpha=0.6, linewidth=2)
        ax2.plot(future_hidden_sizes, flash_lower, '--', color=flash_curve_color, alpha=0.6, linewidth=2)
        
        # Real measurements as reference - use family-consistent colors from the scheme
        ...
        # Real measurements colors (lighter shades for scatter points)
        real_flash_scatter_color = self.color_scheme.get('stateful_secondary', '#cde8e1') # Light green
        real_ram_scatter_color = self.color_scheme.get('windows_secondary', '#afd9dc')    # Light blue
        
        ax2.scatter(real_hs, real_flash_vals, color=real_flash_scatter_color, s=200, marker='^', 
                   label='Flash Real Measurements', zorder=5, edgecolors='black', linewidth=0.5)
        ax2.scatter(real_hs, real_ram_vals, color=real_ram_scatter_color, s=200, marker='^',
                   label='RAM Real Measurements', zorder=5, edgecolors='black', linewidth=0.5)
        
        # Arduino limits - consistent accent color
        limit_color_ax2 = self.color_scheme.get('accent_blue', '#2091C9') # Ensure consistency
        ax2.axhline(y=256, color=limit_color_ax2, linestyle='--', alpha=0.6, linewidth=2, 
                   label='Arduino R4 Flash Limit (256 kB)')
        ax2.axhline(y=32, color=limit_color_ax2, linestyle='--', alpha=0.6, linewidth=2,
                   label='Arduino R4 RAM Limit (32 kB)')
        ...
        if estimation_data:
            ...
            # Flash Pie Chart colors from scheme
            flash_pie_colors = [
                self.color_scheme.get('stateful_secondary', '#cde8e1'), # Light green
                self.color_scheme.get('stateful_lstm', '#b2e1c1')       # Darker green
            ]
            
            wedges, texts, autotexts = ax3.pie(sizes, labels=labels, autopct='%1.1f%%', 
                                             colors=flash_pie_colors, startangle=90,
        ...
        if estimation_data:
            ...
            # RAM Pie Chart colors from scheme
            ram_pie_colors = [
                self.color_scheme.get('windows_secondary', '#afd9dc'),  # Light blue
                self.color_scheme.get('windows_lstm', '#c7dfed')        # Darker blue
            ]
            
            wedges, texts, autotexts = ax4.pie(sizes, labels=labels, autopct='%1.1f%%',
                                             colors=ram_pie_colors, startangle=90,
                                             textprops={'fontsize': 10, 'fontweight': 'bold'})
