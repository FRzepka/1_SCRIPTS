...
class ModelArchitectureAnalyzer:
    ...
    def create_comprehensive_visualization(self):
        ...
        ax1.set_title('Parameter Comparison: Theoretical Formula vs. PyTorch Model\\n(Color-coded by Model Type)', fontweight='bold', fontsize=18) # Increased
        ax1.set_xticks(x)
        ax1.set_xticklabels(architectures, rotation=45, fontsize=13) # Increased
        ...
        ax1.legend(handles=legend_elements, fontsize=10, loc='upper left', ncol=2) # Increased
        ax1.grid(True, alpha=0.3)
        ax1.set_xlabel('Architecture', fontsize=16) # Increased
        ax1.set_ylabel('Parameters Count', fontsize=16) # Increased
            ...
            for bar in bars:
                ...
                if height > 0:
                    ax1.text(bar.get_x() + bar.get_width()/2., height + height*0.02,
                            f'{int(height):,}', ha='center', va='bottom', fontweight='bold', fontsize=11) # Increased
        ...
        for i, arch in enumerate(architectures):
            if model_params[i] > 0:                # Calculate overhead percent
                ...
                ax2.annotate(f'{arch}\\n+{overhead_percent:.1f}%', # Corrected newline
                           (model_params[i], file_sizes[i]), 
                           xytext=(5, 5), textcoords='offset points', fontsize=10) # Increased
        
        ax2.set_xlabel('Number of Parameters', fontsize=16) # Increased
        ax2.set_ylabel('File Size (kB)', fontsize=16) # Increased
        ax2.set_title('File Size vs. Parameter Count\\n(Theoretical vs. Measured)', fontsize=18, fontweight='bold') # Increased
        ...
        ax2.legend(handles=legend_elements, fontsize=10, loc='upper left') # Increased
        ...
        if hidden_sizes and any(h > 0 for h in hidden_sizes):
            ...
            for h, p, arch in zip(valid_hidden, valid_params, valid_archs):
                ax3.annotate(arch, (h, p), xytext=(5, 5), textcoords='offset points', fontsize=11) # Increased
            
            ax3.set_xlabel('Hidden Size', fontsize=16) # Increased
            ax3.set_ylabel('Number of Parameters', fontsize=16) # Increased
            ax3.set_title('Parameter Scaling with Hidden Size', fontsize=18, fontweight='bold') # Increased
            ax3.legend(fontsize=12) # Increased
            ax3.grid(True, alpha=0.3)
        else:
            ax3.text(0.5, 0.5, 'No Hidden Size\\nData Available', 
                    ha='center', va='center', transform=ax3.transAxes, fontsize=16) # Increased
        ...
        ax4.text(0.05, 0.95, summary_text, transform=ax4.transAxes, 
                verticalalignment='top', fontsize=11, fontfamily='monospace', # Increased
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        if ram_real_filtered:
            # Use medium blue for real RAM measurements - your diagram style  
            ...
            bars4 = ax1.bar(x[:len(ram_real_filtered)] + width*1.5, ram_real_filtered, width,
                           ...
                           edgecolor='black', linewidth=1)  # Black edge# Arduino limits - use elegant soft colors from your diagram
        ...
        ax1.set_title('Arduino Flash/RAM Estimation vs. Real Measurements', fontsize=18, fontweight='bold') # Increased
        ax1.set_xticks(x)
        ax1.set_xticklabels([f'{arch}\\n({hs}×{hs})' for arch, hs in zip(architectures, hidden_sizes)], fontsize=13) # Increased
        ax1.legend(fontsize=12) # Increased
        ax1.grid(True, alpha=0.3)
        ax1.set_xlabel('Architecture', fontsize=16) # Increased
        ...
        ax2.set_title('Logarithmic Scaling: Flash/RAM vs. Hidden Size\\n(with continuous uncertainty ranges)', fontsize=18, fontweight='bold') # Increased
        ax2.legend(loc='upper left', fontsize=10) # Increased
        ax2.grid(True, alpha=0.3)
        ...
        if estimation_data:
            ...
            labels = [label.replace('_', ' ').title() for label in flash_components.keys()]
              # Use elegant colors from mint_blue diagram for Flash
            colors_pie = ['#A8E6CF', '#C8F0DB']  # Mint green palette from mint_blue
            
            wedges, texts, autotexts = ax3.pie(sizes, labels=labels, autopct='%1.1f%%', 
                                             colors=colors_pie, startangle=90,
                                             textprops={'fontsize': 14, 'fontweight': 'bold'}) # Increased label fontsize
            ax3.set_title(f'Flash Components Breakdown\n({architectures[0]} Architecture)', 
                         fontsize=16, fontweight='bold') # Increased
        ...
        if estimation_data:
            ...
            wedges, texts, autotexts = ax4.pie(sizes, labels=labels, autopct='%1.1f%%',
                                             colors=colors_pie, startangle=90,
                                             textprops={'fontsize': 14, 'fontweight': 'bold'}) # Increased
    ...
    def create_mcu_overview_visualization(self):
        ...
        ax.set_title('Microcontroller Overview: Memory Requirements vs. MCU Capabilities', 
                     fontsize=24) # Increased fontsize, removed bold
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.grid(True, which="both", ls="-", alpha=0.3) # Grid for both major and minor ticks on log scale
        ax.legend(fontsize=16, loc='upper left', bbox_to_anchor=(1,1)) # Increased fontsize and adjusted legend position
