...
class ModelArchitectureAnalyzer:
    ...
    def create_comprehensive_visualization(self):
        ...
        theoretical_color = self.color_scheme['stateful_lstm']  # Soft mint green for theoretical
        measured_color = self.color_scheme['stateful_32']     # Medium mint green for measured
    ...
    def create_arduino_memory_estimation_visualization(self):
        ...
        if estimation_data:
            ...
            labels = [f"{label.replace('_', ' ').title()}\n({size:.1f} kB)" for label, size in zip(flash_components.keys(), sizes)]            # Use elegant colors from mint_blue diagram for Flash
        ...
        if estimation_data:
            ...
            labels = [f"{label.replace('_', ' ').title()}\n({size:.1f} kB)" for label, size in zip(ram_components.keys(), sizes)]
