...
def create_dataloaders(datasets, seq_len, batch_size):
    ...
    for key, df in datasets.items():
        if not df.empty and ('train' in key or 'val' in key or 'test' in key):
            ...
            # Add num_workers=0 to prevent multiprocessing issues that can cause hanging
            loader = DataLoader(ds, batch_size=batch_size, shuffle=('train' in key), 
                              num_workers=0, pin_memory=False)
            loaders[key] = loader
            print(f"Created DataLoader for {key}: {len(ds)} samples, {len(loader)} batches")
    return loaders
...
class DataProcessor:
    ...
    def load_cell_data(self):
        # Angepasst für MGFarm_18650_CXX Ordnerstruktur
        folders = sorted([d for d in self.data_dir.iterdir() if d.is_dir() and d.name.startswith('MGFarm_18650_C')],
                        key=lambda x: int(x.name.split('_C')[-1]))
        info = {}
        for folder in folders:
            cell_id = folder.name.split('_C')[-1].zfill(2)  # z.B. "01", "03", etc.
            parquet_file = folder / 'df.parquet'
            if parquet_file.exists():
                info[cell_id] = parquet_file
            else:
                logger.warning(f"df.parquet not found in {folder}")
    ...
    def process_file(self, fp):
        ...
        # Angepasst für MGFarm Datenstruktur - prüfe verfügbare Spalten
        available_cols = df.columns.tolist()
        logger.info(f"Available columns in {fp.name}: {available_cols}")
        
        # Versuche die benötigten Spalten zu finden
        col_mapping = {
            'Testtime[s]': ['Testtime[s]', 'Time[s]', 'Absolute_Time[yyyy-mm-dd hh:mm:ss]'],
            'Voltage[V]': ['Voltage[V]', 'U[V]', 'Voltage'],
            'Current[A]': ['Current[A]', 'I[A]', 'Current'], 
            'Temperature[°C]': ['Temperature[°C]', 'T[°C]', 'Temperature'],
            'SOH_ZHU': ['SOH_ZHU', 'SOH']
        }
        ...
        for target_col, possible_names in col_mapping.items():
            ...
            for possible_name in possible_names:
                if possible_name in available_cols:
                    final_cols[target_col] = possible_name
                    found = True
                    break
            if not found:
                logger.warning(f"Column {target_col} not found in {fp.name}")
                
        # Verwende gefundene Spalten
        df_subset = df[[col for col in final_cols.values()]].copy()
        df_subset.columns = list(final_cols.keys())
        
        df_subset = df_subset.dropna().reset_index(drop=True)
        
        # Zeitstempel verarbeiten
        if 'Testtime[s]' in df_subset.columns:
            df_subset['Testtime[s]'] = df_subset['Testtime[s]'].round().astype(int)
        else:
            # Falls keine Testtime verfügbar, erstelle eine
            df_subset['Testtime[s]'] = range(len(df_subset))
            
        df_subset['Datetime'] = pd.date_range('2023-02-02', periods=len(df_subset), freq='s')
        df_subset = df_subset.set_index('Datetime').resample(self.resample).mean().reset_index()
        df_subset['cell_id'] = fp.parent.name.split('_C')[-1].zfill(2)  # Extrahiere Cell ID aus Ordnername
        return df_subset
...
class Trainer:
    ...
    def train_task(self, train_loader, val_loader, task_id, apply_ewc=True, resume=False):
        ...
        print(f"⚙️ Apply EWC: {apply_ewc}, Resume: {resume}")
        ...
        if resume and ckpt_last.exists():
            print(f"📂 Loading checkpoint from {ckpt_last}")
        ...
        print(f"🔄 Starting training loop from epoch {start_epoch} to {self.config.EPOCHS}")
        for epoch in tqdm.tqdm(range(start_epoch, self.config.EPOCHS), desc="Training Epochs", position=0):
            epoch_start = time.time() 
            logger.info("Epoch %d/%d", epoch+1, self.config.EPOCHS)
            self.model.train()
            train_loss = 0
            
            # Add batch-level progress bar for training
            train_pbar = tqdm.tqdm(train_loader, desc=f"Epoch {epoch+1} - Training", position=1, leave=False)
            for batch_idx, (x, y) in enumerate(train_pbar):
                ...
                if apply_ewc and self.ewc_tasks:
                    ewc_loss = (self.config.EWC_LAMBDA/2)*sum(t.penalty(self.model) for t in self.ewc_tasks)
                    loss += ewc_loss
                loss.backward()
                ...
                avg_loss = train_loss / ((batch_idx + 1) * x.size(0))
                train_pbar.set_postfix({'batch_loss': f'{loss.item():.4f}', 'avg_loss': f'{avg_loss:.4f}'})
            train_pbar.close()
            ...
            val_loss = 0
            with torch.no_grad():
                val_pbar = tqdm.tqdm(val_loader, desc=f"Epoch {epoch+1} - Validation", position=1, leave=False)
                for x, y in val_pbar:
                    x, y = x.to(self.device), y.to(self.device)
                    batch_val_loss = F.mse_loss(self.model(x), y).item() * x.size(0)
                    val_loss += batch_val_loss
                    val_pbar.set_postfix({'val_loss': f'{batch_val_loss/x.size(0):.4f}'})
                val_pbar.close()
            ...
            if val_loss < best_val:
                ...
                print(f"💾 Saved best model at epoch {epoch+1} with val_loss: {best_val:.6f}")
            else:
                no_imp += 1
    ...
    def consolidate(self, loader):
        print(f"🔄 Consolidating EWC for {len(loader)} batches...")
        self.ewc_tasks.append(EWC(self.model, loader, self.device))
        print(f"✅ EWC consolidation complete. Total EWC tasks: {len(self.ewc_tasks)}")
...
def main(skip_regular=True):
    # config and logging
    config = Config()
    base_dir = Path(__file__).parent / 'models' / 'seq5days_new_datasplit2_robust'
    # ---- regular dirs ----
    ...
    if not skip_regular:
        ...
        dp_lstm = DataProcessor(
            data_dir=r'c:\Users\Florian\SynologyDrive\TUB\3_Projekte\MG_Farm\5_Data\01_LFP\00_Data\MGFarm_18650_Dataframes_ZHU',
