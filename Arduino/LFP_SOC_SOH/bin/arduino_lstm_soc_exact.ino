...
float lstm_forward(float input[INPUT_SIZE]) {
  ...
  for (int i = 0; i < MLP_HIDDEN_SIZE; i++) {
    mlp1_out[i] = mlp_layer0_bias[i];
    for (int j = 0; j < ARDUINO_HIDDEN_SIZE; j++) {
      mlp1_out[i] += mlp_layer0_weights[i][j] * h[j];
    }
  ...
  for (int i = 0; i < MLP_HIDDEN_SIZE; i++) {
    mlp2_out[i] = mlp_layer3_bias[i];
    for (int j = 0; j < MLP_HIDDEN_SIZE; j++) {
      mlp2_out[i] += mlp_layer3_weights[i][j] * mlp1_out[j];
    }
    mlp2_out[i] = relu_activation(mlp2_out[i]);
  }
  
  // Layer 3: Linear(32, 1) + Sigmoid
  float soc_output = mlp_layer6_bias[0];
  for (int j = 0; j < MLP_HIDDEN_SIZE; j++) {
    soc_output += mlp_layer6_weights[0][j] * mlp2_out[j];
  }
