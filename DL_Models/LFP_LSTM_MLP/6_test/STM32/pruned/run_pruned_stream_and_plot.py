#!/usr/bin/env python3
"""
STM32 PRUNED Model: stream 6 features for N samples over UART, collect SOC predictions,
compare against ground-truth from a parquet file, and save plots + metrics.

Outputs are saved under:
  DL_Models/LFP_LSTM_MLP/6_test/STM32/pruned/STM32_PRUNED_STREAM_YYYYMMDD_HHMMSS
"""
import argparse
import json
import re
...
DEFAULT_FEATURES = [
    'Voltage[V]', 'Current[A]', 'Temperature[��C]', 'Q_c', 'dU_dt[V/s]', 'dI_dt[A/s]'
]
...
def main():
    ap = argparse.ArgumentParser(description='STM32 PRUNED: stream and plot')
    ...
    ap.add_argument('--yaml', default=str(DEFAULT_YAML), help='YAML config (reads model.features if present)')
    ...
    ap.add_argument('--reset-state', action='store_true', help='Send RESET before streaming')
    args = ap.parse_args()
    ...
    out_dir = SCRIPT_DIR / f'STM32_PRUNED_STREAM_{ts}'
    ...
    diffs = np.abs(stm_preds[valid] - y_true[valid]) if valid.any() else np.array([], dtype=np.float32)
    ...
    with open(out_dir / 'metrics.json', 'w', encoding='utf-8') as f:
        json.dump({
            'N': int(end - args.start), 'valid': int(valid.sum()), 'timeouts': int(timeouts),
            'MAE_vs_GT': mae, 'RMSE_vs_GT': rmse, 'MAX_vs_GT': mxd,
            'features': cols, 'parquet': str(Path(args.parquet).resolve()),
            'yaml': str(Path(args.yaml).resolve()) if args.yaml else '', 'port': args.port, 'baud': args.baud,
        }, f, indent=2)

    plt.figure(figsize=(12, 4))
    plt.plot(y_true, label='GT', alpha=0.6)
    plt.plot(stm_preds, label='STM32_PRUNED', alpha=0.8)
    ...
    plt.plot(stm_preds[:firstN], label='STM32_PRUNED', alpha=0.8)
    plt.legend(); plt.tight_layout(); plt.savefig(out_dir / 'overlay_firstN.png', dpi=150); plt.close()

    if valid.any():
        plt.figure(figsize=(6, 4))
        plt.hist(diffs, bins=100, alpha=0.8, color='tab:purple', edgecolor='black')
        plt.title('Abs error vs GT (STM32 PRUNED)')
