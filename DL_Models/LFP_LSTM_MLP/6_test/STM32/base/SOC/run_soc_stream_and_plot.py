#!/usr/bin/env python3
"""
STM32 Base Model: stream 6 features for N samples over UART, collect SOC predictions,
compare against ground-truth from a parquet file, and save plots + metrics.

Outputs are saved under:
  DL_Models/LFP_LSTM_MLP/6_test/STM32/base/SOC/results/STM32_SOC_STREAM_YYYYMMDD_HHMMSS
"""
import argparse
import json
import re
...
def main():
    ap = argparse.ArgumentParser(description='STM32 Base Model 5000-step stream and plot')
    ...
    # Save in results folder: .../6_test/STM32/base/SOC/results/STM32_SOC_STREAM_*
    out_dir = SCRIPT_DIR / 'results' / f'STM32_SOC_STREAM_{ts}'
    ...
    with serial.Serial(args.port, args.baud, timeout=args.timeout) as ser, open(log_path, 'w', encoding='utf-8') as logf:
        ...
        if boot:
            print(boot.strip())
            logf.write(boot)
        # Optional: reset LSTM state on device
        if args.reset_state:
            ser.write(b'RESET\n')
            ser.flush()
            time.sleep(0.05)
            # Drain any immediate ack
        ...
        for i, idx in enumerate(range(args.start, end)):
            ...
            # Read until we see SOC or ERR or timeout
            ...
            while time.time() < deadline:
                ...
                if text:
                    parts.append(text)
                # Only accept SOC lines to avoid parsing DBG/READY
                p = extract_soc(text)
                if p is not None:
                    pred = p
                    break
    ...
    plt.plot(stm_preds, label='STM32_BASE', alpha=0.8)
    ...
    plt.plot(stm_preds[:firstN], label='STM32_BASE', alpha=0.8)
    ...
    if valid.any():
        ...
        plt.title('Abs error vs GT (STM32 Base)')
