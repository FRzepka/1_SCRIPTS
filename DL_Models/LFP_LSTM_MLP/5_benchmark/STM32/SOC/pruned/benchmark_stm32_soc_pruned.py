#!/usr/bin/env python3
"""
STM32 SOC Host Benchmark (Pruned)
...
Usage example (Windows cmd):
  conda activate ml1
  pip install -r STM32\benchmark\SOC_pruned\requirements.txt
  python STM32\benchmark\SOC_pruned\benchmark_stm32_soc_pruned.py --port COM7 --samples 5000
...
def run_benchmark(port, baud, samples, rate_delay, timeout, outdir, seed=None, data_path=None):
    ...
    # Path relative to STM32/benchmark/SOC_pruned/
    map_path = os.path.join('..', '..', 'workspace_1.17.0', 'AI_Project_LSTM_SOC_pruned', 'Debug', 'AI_Project_LSTM_SOC_pruned.map')
    map_path = os.path.normpath(os.path.join(os.path.dirname(__file__), map_path))
    elf_path = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', '..', 'workspace_1.17.0', 'AI_Project_LSTM_SOC_pruned', 'Debug', 'AI_Project_LSTM_SOC_pruned.elf'))
    ...
    md_table = f"""
### Benchmark Results Summary (Pruned)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--outdir', default=None, help='Output directory (defaults to DL_Models/LFP_LSTM_MLP/5_benchmark/STM32/SOC/pruned/results_TIMESTAMP)')
    ...
    if args.outdir is None:
        ...
        base_out = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'DL_Models', 'LFP_LSTM_MLP', '5_benchmark', 'STM32', 'SOC', 'pruned')
