...
def run_benchmark(model_name, n_samples=1000):
    ...
    # Auto-Reset via DTR (ASCII only to avoid Windows console encoding issues)
                ...
                if line:
                    if "RAM_MEASURE" in line:
                        ...
                        if m:
                            ...
                            print(f"Captured Static RAM from boot: {metrics['static_ram']} bytes")
                            boot_captured = True
        except Exception as e:
    ...
    for i in range(n_samples):
        ...
        # METRICS and SOH prediction have been received.
        t_start = time.time()
        ...
        while (not got_metrics or not got_pred) and (time.time() - start_wait < 2.0):
            if ser.in_waiting:
                ...
                if "METRICS:" in line:
                    ...
                    # If boot capture failed, we can still pick up static RAM from METRICS line
                    if m_static:
