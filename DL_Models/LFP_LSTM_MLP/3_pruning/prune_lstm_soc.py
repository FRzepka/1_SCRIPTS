...
# -------------------------
# Dataset utilities (minimal reuse from training)
# -------------------------
...
def eval_model(model, loader, device, microbatch: int = None, progress: bool = False, desc: str = "eval"):
    ...
    try:
        ...
        for xb, yb in iterator:
            # micro-batch on device to reduce peak memory
            if device.type == 'cuda':
                xb = xb.to(device, non_blocking=True)
            # Split into microbatches if requested
        ...
        # Fallback to CPU if we hit CUDA OOM
        if 'CUDA out of memory' in str(e):
            ...
            preds, ys = [], []
            iterator = tqdm(loader, total=len(loader), desc=f"{desc}-cpu", leave=False) if progress else loader
            for xb, yb in iterator:
                pred = model_cpu(xb.to(device_cpu))
...
def lstm_unit_saliency(lstm: nn.LSTM) -> np.ndarray:
    ...
    # reshape to [4, H, in] and [4, H, H]
    Wih_g = Wih.reshape(4, H, -1)
    Whh_g = Whh.reshape(4, H, H)
    # L2 per unit over inputs/hidden, sum across gates and sources
...
def prune_lstm_mlp(model: LSTMMLP, keep_idx: np.ndarray) -> LSTMMLP:
    """Create a new model with reduced hidden_size, copying the selected units.
    Supports num_layers==1.
    """
    old = model
    lstm = old.lstm
    assert lstm.num_layers == 1, "This pruning script currently supports num_layers==1"
    ...
    # Build new modules
    new_model = LSTMMLP(in_features=in_f, hidden_size=Hn, mlp_hidden=old.mlp[0].out_features,
                        num_layers=1, dropout=0.05)

    # Copy LSTM weights (layer 0)
    with torch.no_grad():
        # weight_ih: rows per gate, keep columns all (input)
        ...
        for g in range(4):
            ...
            Whh_g_rows = gate_slice(Whh, g, keep_idx)  # rows select outputs
            # For Whh, also need to select columns corresponding to kept hidden units
        ...
        for xb, yb in tbar:
            ...
            if microbatch and xb.size(0) > microbatch:
                ...
                for xb_mb, yb_mb in zip(torch.split(xb, microbatch, dim=0), torch.split(yb, microbatch, dim=0)):
                    ...
                    mb_losses.append(loss_mb.detach().item() * xb_mb.size(0))
                    if scaler is not None:
                        scaler.scale(loss_mb).backward()
                    else:
                        loss_mb.backward()
                # optimizer step once per full batch
                if scaler is not None:
                    if max_grad_norm:
                        scaler.unscale_(opt)
...
def main():
    ap = argparse.ArgumentParser(description='Structured LSTM unit-pruning for SOC model (1.5.0.0)')
    ...
    ap.add_argument('--prune-ratio', type=float, default=0.5, help='Fraction of hidden units to remove (0..1)')
    ...
    ap.add_argument('--device', type=str, default='auto', choices=['auto', 'cuda', 'cpu'], help='Device to use for pruning/finetune')
    ap.add_argument('--train-batch', type=int, default=None, help='Override training batch size')
    ap.add_argument('--val-batch', type=int, default=None, help='Override validation batch size')
    ap.add_argument('--microbatch', type=int, default=None, help='Optional micro-batch size to reduce peak CUDA memory')
    ap.add_argument('--eval-microbatch', type=int, default=None, help='Optional micro-batch size used only for eval')
    args = ap.parse_args()
    ...
    print(f"[config] device={device.type} | prune_ratio={args.prune_ratio} | finetune_epochs={args.finetune_epochs}")
    ...
    # Rebuild and load baseline model
    ...
    # Data
    # Batch-size selection and pin_memory based on device
    ...
    try:
        print(f"[data] train_bs={train_bs} | val_bs={val_bs} | pin_memory={pin_memory}")
        print(f"[data] train_batches={len(train_loader)} | val_batches={len(val_loader)}")
    except Exception:
        pass

    # Baseline metrics
    print("[eval] Baseline evaluation...")
    ...
    print(f"[prune] hidden_size {H} -> {keep} (remove {remove})")

    # Prune
    pruned = prune_lstm_mlp(base, keep_idx)
    pruned.to(device)
    print("[prune] Model pruned and moved to device")

    # Finetune
    print(f"[finetune] Starting fine-tune for {args.finetune_epochs} epoch(s), lr={args.lr}, microbatch={args.microbatch}, eval_microbatch={args.eval_microbatch}")
