...
void lstm_model_soh_int8_inference_batch(float* input_flat, LSTMStateSOH* state, float* output, int n_samples)
{
    for (int i = 0; i < n_samples; i++) {
        float* in_ptr = &input_flat[i * INPUT_SIZE];
        output[i] = lstm_model_soh_int8_forward(in_ptr, state);
    }
}
