#!/usr/bin/env python3
"""
0.4.1.2 SOH Training – Hourly Aggregation + Stats (Seq2Seq, Causal CNN)
Key updates:
...
- Output smoothing conv + stronger smooth loss to reduce noise
"""
import os
import json
import math
...
class SOH_CNN_Seq2Seq(nn.Module):
    ...
    def __init__(
        ...
        if self.output_kernel_size > 1:
            self.output_smoother = CausalConv1d(1, 1, self.output_kernel_size, dilation=1)
        else:
            self.output_smoother = None
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description='0.4.1.2 SOH Training – Hourly Aggregation + Stats (Seq2Seq, Causal CNN)')
    ...
    output_kernel_size = int(cfg['model'].get('output_kernel_size', 1))
    ...
    if warmup_steps < 0:
        warmup_steps = max(0, min(model.receptive_field - 1, chunk - 1))
        cfg['training']['warmup_steps'] = warmup_steps

    print('\n' + '=' * 80)
    print('0.4.1.2 SOH Training – Hourly Aggregation + Stats (Seq2Seq, Causal CNN)')
    print('=' * 80)
    ...
    if dilations is not None:
        print(
            f'Model: hidden={hidden_size}, dilations={dilations}, kernel={kernel_size}, '
            f'out_kernel={output_kernel_size}, mlp={mlp_hidden}'
        )
    else:
        print(
            f'Model: hidden={hidden_size}, blocks={num_blocks}, kernel={kernel_size}, '
            f'out_kernel={output_kernel_size}, mlp={mlp_hidden}'
        )
