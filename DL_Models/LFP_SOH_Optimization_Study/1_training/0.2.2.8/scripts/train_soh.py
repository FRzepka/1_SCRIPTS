#!/usr/bin/env python3
"""
0.2.2.1 SOH Training – Fast Causal TCN (Seq2Seq, Hourly Aggregation)
Key updates:
- Causal dilated residual TCN (streaming-friendly)
- Predicts SOH at every timestep (seq2seq)
- Hourly aggregation with mean/std/min/max per feature
- Optional warmup_steps to ignore receptive-field startup
"""
import os
import json
import math
...
def make_scheduler(optimizer, cfg):
    ...
    if stype == 'cosine_warm_restarts':
        ...
        eta_min_factor = float(sched_cfg.get('eta_min_factor', 0.05))
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description='0.2.2.1 SOH Training – Fast Causal TCN Seq2Seq (Hourly Aggregation)')
    ...
    cudnn_benchmark = bool(perf_cfg.get('cudnn_benchmark', not deterministic))
    ...
    if warmup_steps < 0:
        warmup_steps = max(0, model.receptive_field - 1)

    print('\n' + '=' * 80)
    print('0.2.2.1 SOH Training – Fast Causal TCN Seq2Seq (Hourly Aggregation)')
    print('=' * 80)
    ...
    print(f'Model: hidden={hidden_size}, kernel={kernel_size}, dilations={dilations}')
    print(f'Receptive field: {model.receptive_field} (warmup_steps={warmup_steps})')
    ...
    print(f'Perf: deterministic={deterministic}, cudnn_benchmark={cudnn_benchmark}, tf32={allow_tf32}, matmul_precision={matmul_precision}')
    print('=' * 80 + '\n')
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (epoch % val_interval == 0) or (epoch == 1) or (epoch == epochs):
            ...
            train_loss_val = float(train_loss) if train_loss is not None else float('nan')
            rmse_str = f'{rmse_val:.6f}' if rmse_val is not None else 'nan'
            mae_str = f'{mae_val:.6f}' if mae_val is not None else 'nan'
            r2_str = f'{r2_val:.4f}' if r2_val is not None else 'nan'

            print(
                f'\n[Epoch {epoch:03d}] Train Loss: {train_loss_val:.6f} | '
                f'Val RMSE: {rmse_str} | MAE: {mae_str} | '
                f'R2: {r2_str} | LR: {current_lr:.2e}\n'
            )
            ...
            if y_true is not None and (epoch % int(cfg.get('tracking', {}).get('plot_interval', 10)) == 0 or epoch == 1):
