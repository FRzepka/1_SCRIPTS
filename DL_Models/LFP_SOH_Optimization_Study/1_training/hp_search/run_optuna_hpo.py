#!/usr/bin/env python3
"""Optuna hyperparameter search for SOH base models (LSTM/TCN/GRU/CNN)."""
...
MODEL_SPECS = {
    "LSTM_0.1.2.3": {"family": "LSTM", "version": "0.1.2.3"},
    "TCN_0.2.2.1": {"family": "TCN", "version": "0.2.2.1"},
    "GRU_0.3.1.1": {"family": "GRU", "version": "0.3.1.1"},
    "CNN_0.4.1.1": {"family": "CNN", "version": "0.4.1.1"},
}
...
def trial_cfg_update(trial: optuna.Trial, cfg: dict, family: str, args) -> dict:
    ...
    c["training"]["max_grad_norm"] = trial.suggest_float("max_grad_norm", 0.6, 1.5)
    c["training"]["seq_chunk_size"] = trial.suggest_categorical("seq_chunk_size", [96, 120, 144, 168, 192, 240])
    ...
    c["training"]["lr"] = trial.suggest_float("lr", 5e-5, 5e-4, log=True)
    c["training"]["weight_decay"] = trial.suggest_float("weight_decay", 1e-6, 5e-4, log=True)
    c["training"]["batch_size"] = trial.suggest_categorical("batch_size", [64, 96, 128])
    c["training"]["smooth_loss_weight"] = trial.suggest_float("smooth_loss_weight", 0.0, 0.35)
    c["training"]["smooth_loss_type"] = trial.suggest_categorical("smooth_loss_type", ["l1", "l2"])
    c["training"]["warmup_steps"] = trial.suggest_categorical("warmup_steps", [0, 4, 8, 12, 16])
    c["training"]["warmup_epochs"] = trial.suggest_int("warmup_epochs", 0, 5)

    if family == "LSTM":
        c["model"]["embed_size"] = trial.suggest_categorical("embed_size", [96, 128, 160])
        c["model"]["hidden_size"] = trial.suggest_categorical("hidden_size", [160, 192, 224, 256])
        c["model"]["mlp_hidden"] = trial.suggest_categorical("mlp_hidden", [128, 160, 192, 224])
        c["model"]["num_layers"] = trial.suggest_int("num_layers", 2, 4)
        c["model"]["res_blocks"] = trial.suggest_int("res_blocks", 1, 3)
        c["model"]["dropout"] = trial.suggest_float("dropout", 0.10, 0.30)
        c["model"]["bidirectional"] = False
    elif family == "GRU":
        c["model"]["embed_size"] = trial.suggest_categorical("embed_size", [96, 128, 160, 192])
        c["model"]["hidden_size"] = trial.suggest_categorical("hidden_size", [160, 192, 224, 256, 320])
        c["model"]["mlp_hidden"] = trial.suggest_categorical("mlp_hidden", [128, 160, 192, 224])
        c["model"]["num_layers"] = trial.suggest_int("num_layers", 2, 5)
        c["model"]["res_blocks"] = trial.suggest_int("res_blocks", 1, 4)
        c["model"]["dropout"] = trial.suggest_float("dropout", 0.10, 0.32)
        c["model"]["bidirectional"] = False
    elif family == "TCN":
        c["model"]["hidden_size"] = trial.suggest_categorical("hidden_size", [64, 96, 128, 160])
        c["model"]["mlp_hidden"] = trial.suggest_categorical("mlp_hidden", [64, 96, 128, 160])
        c["model"]["kernel_size"] = trial.suggest_categorical("kernel_size", [3, 5])
        c["model"]["dropout"] = trial.suggest_float("dropout", 0.02, 0.20)
        dilations_label = trial.suggest_categorical(
            "dilations",
            ["1,2,4,8", "1,2,4,8,16", "1,2,4,8,16,32", "1,2,4,4,8,16"],
        )
        c["model"]["dilations"] = [int(x) for x in dilations_label.split(",")]
        c["training"]["warmup_steps"] = trial.suggest_categorical("warmup_steps_tcn", [-1, 0, 8, 16, 24, 32])
    elif family == "CNN":
        c["model"]["hidden_size"] = trial.suggest_categorical("hidden_size", [128, 160, 192, 224, 256])
        c["model"]["mlp_hidden"] = trial.suggest_categorical("mlp_hidden", [96, 128, 160, 192, 224])
        c["model"]["kernel_size"] = trial.suggest_categorical("kernel_size", [3, 5])
        c["model"]["dropout"] = trial.suggest_float("dropout", 0.08, 0.25)
        dilations_label = trial.suggest_categorical(
            "dilations",
            ["1,2,4,8", "1,2,4,8,16", "1,1,2,4,8", "1,2,2,4,8"],
        )
        c["model"]["dilations"] = [int(x) for x in dilations_label.split(",")]
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run Optuna HPO for one SOH base model.")
    ...
    ap.add_argument("--early-stopping", type=int, default=12)
