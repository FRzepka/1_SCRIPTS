...
export ROOT TRAIN_ROOT PY SPECS_JSON TS LOG_DIR
...
target_roles = {"smaller_3", "larger_3"}
...
for idx, family_spec in enumerate(specs["families"]):
    ...
    commands = ["set -euo pipefail", f"cd '{root}'", f"sleep {idx * 20}"]
    picked = [v for v in family_spec["variants"] if v["role"] in target_roles]
    for variant in picked:
        ...
        commands.append(f"echo '[start] {family} {version} {tag}' | tee -a '{family_log}'")
        commands.append(
...
with open(log_dir / "SESSIONS.txt", "w", encoding="utf-8") as f:
    ...
    f.write(f"mode: edge-extension\n")
    f.write(f"specs: {specs_json}\n")
    f.write(f"roles: {','.join(sorted(target_roles))}\n")
