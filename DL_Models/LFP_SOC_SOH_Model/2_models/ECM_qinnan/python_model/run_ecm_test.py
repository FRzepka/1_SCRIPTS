...
def main():
    ...
    ap.add_argument("--native_lib", default=None, help="Path to libecm_ekf.so (native)")
    ...
    if ecm_param_c is None:
        ecm_param_c = os.path.join(
            os.path.dirname(__file__),
            "..", "simulator", "ECM_parameter.c"
        )
    ...
    if args.native:
        ...
        model = ECMNativeEKF(lib_path=args.native_lib, soc_init=float(args.soc_init))
    else:
        raise SystemExit(
