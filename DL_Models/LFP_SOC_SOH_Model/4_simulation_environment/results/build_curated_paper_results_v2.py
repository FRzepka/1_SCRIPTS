...
PAPER_TABLES_DIR = RESULTS_DIR / "paper_tables_v2"
PAPER_FIGURES_DIR = RESULTS_DIR / "paper_figures_v2"
LOCAL_ANALYSIS_DIR = ROOT / "analysis_local_focus" / "2026-03-24_local_recovery_v2"
NOISE_DETAIL_DIR = RESULTS_DIR / "noise_detail_v2"
CAMPAIGN_DEFAULT = "2026-03-12_extended_matrix_fullc07"
BIAS_CAMPAIGN_TAG = "bias_0p5_1p5_3p0_percent_fullc07_v2_soc170"
BIAS_ECM_CAMPAIGN_TAG = "bias_0p5_1p5_3p0_percent_fullc07_v2_soc170"
...
from build_paper_results_v2 import load_campaign_rows
...
def figure_noise(df: pd.DataFrame) -> None:
    ...
    detail_csv = NOISE_DETAIL_DIR / "current_noise_detail.csv"
    jitter_csv = NOISE_DETAIL_DIR / "current_noise_output_jitter.csv"
...
def build_readme(df: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "",
        "- `paper_tables_v2/table_key_results.md` contains the complete **global** scenario metrics used for ranking.",
        "- `paper_tables_v2/table_local_behaviour.md` contains the **local** recovery and drift metrics used when global MAE is not informative enough.",
        "- `paper_tables_v2/table_figure_scope.md` maps every paper figure to its global and local evidence.",
    ...
    (RESULTS_DIR / "PAPER_RESULTS_v2.md").write_text("\n".join(lines) + "\n")


def main() -> None:
    global PAPER_TABLES_DIR, PAPER_FIGURES_DIR, LOCAL_ANALYSIS_DIR, NOISE_DETAIL_DIR, BIAS_CAMPAIGN_TAG, BIAS_ECM_CAMPAIGN_TAG
    ap = argparse.ArgumentParser()
    ap.add_argument("--campaign_tag", default=CAMPAIGN_DEFAULT)
    ap.add_argument("--cell", default="MGFarm_18650_C07")
    ap.add_argument("--paper_tables_dir", default=None)
    ap.add_argument("--paper_figures_dir", default=None)
    ap.add_argument("--local_analysis_dir", default=None)
    ap.add_argument("--noise_detail_dir", default=None)
    ap.add_argument("--bias_campaign_tag", default=BIAS_CAMPAIGN_TAG)
    ap.add_argument("--bias_ecm_campaign_tag", default=BIAS_ECM_CAMPAIGN_TAG)
    ...
    if args.local_analysis_dir:
        LOCAL_ANALYSIS_DIR = Path(args.local_analysis_dir)
    if args.noise_detail_dir:
        NOISE_DETAIL_DIR = Path(args.noise_detail_dir)
    BIAS_CAMPAIGN_TAG = args.bias_campaign_tag
    BIAS_ECM_CAMPAIGN_TAG = args.bias_ecm_campaign_tag
    ...
    manifest_out = Path(args.manifest_out) if args.manifest_out else (RESULTS_DIR / "paper_results_manifest_v2.json")
