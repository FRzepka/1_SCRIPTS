...
EXT_TAG = "2026-03-27_extended_matrix_fullc07_v6_soc171_s30ft_soh0125_s30ft"
BIAS_TAG = "bias_0p5_1p5_3p0_percent_fullc07_v6_soc171_s30ft_soh0125_s30ft"
NOISE_TAG = "2026-03-27_current_noise_detail_fullc07_v6_soc171_s30ft_soh0125_s30ft"
LOCAL_OUT = ROOT / "analysis_local_focus" / "2026-03-27_local_recovery_v6"
NOISE_OUT = ROOT / "results" / "noise_detail_v6"
PAPER_TABLES_OUT = ROOT / "results" / "paper_tables_v6"
PAPER_FIGURES_OUT = ROOT / "results" / "paper_figures_v6"
PAPER_MANIFEST_OUT = ROOT / "results" / "paper_results_manifest_v6.json"
CELL = "MGFarm_18650_C07"

SOC_CONFIG = Path("/home/florianr/MG_Farm/1_Scripts/DL_Models/LFP_SOC_SOH_Model/2_models/SOC_1.7.1.0/PrunedFT_1.7.1.0_s30_struct/config/train_soc.yaml")
SOC_CKPT = Path("/home/florianr/MG_Farm/1_Scripts/DL_Models/LFP_SOC_SOH_Model/2_models/SOC_1.7.1.0/PrunedFT_1.7.1.0_s30_struct/checkpoints/best_model_finetuned.pt")
SOC_SCALER = Path("/home/florianr/MG_Farm/1_Scripts/DL_Models/LFP_SOC_SOH_Model/2_models/SOC_1.7.1.0/PrunedFT_1.7.1.0_s30_struct/scaler_robust.joblib")
...
def main() -> None:
    ...
    jobs = {
        ...
        "benchv6_soc171": (
            ...
            f"> {shlex.quote(str(campaign_dir / 'SOC_SOH_1.7.1.0_0.1.2.5_extended.log'))} 2>&1 && "
            ...
            f"> {shlex.quote(str(campaign_dir / 'SOC_SOH_1.7.1.0_0.1.2.5_bias.log'))} 2>&1 && "
            ...
            f"> {shlex.quote(str(campaign_dir / 'SOC_SOH_1.7.1.0_0.1.2.5_noise.log'))} 2>&1"
        ),
    }
    ...
    print("[POST] local recovery analysis v6", flush=True)
    subprocess.run(
        [
            str(PYTHON),
    ...
    print("[POST] current-noise detail v6", flush=True)
    subprocess.run(
    ...
    print("[POST] curated paper outputs v6", flush=True)
    subprocess.run(
        [
            str(PYTHON),
    ...
    print("[DONE] benchmark v6 complete", flush=True)
