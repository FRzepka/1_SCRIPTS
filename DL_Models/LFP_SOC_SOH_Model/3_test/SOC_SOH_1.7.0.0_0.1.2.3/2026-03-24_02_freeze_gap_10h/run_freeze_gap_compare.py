...
def freeze_gap(df: pd.DataFrame, start_idx: int, gap_len: int, cols_to_freeze: List[str]) -> pd.DataFrame:
    out = df.copy()
    end_idx = min(start_idx + gap_len, len(out))
    if start_idx <= 0 or start_idx >= len(out):
        return out
    prev = out.iloc[start_idx - 1]
    for col in cols_to_freeze:
        if col in out.columns:
            out.loc[start_idx:end_idx - 1, col] = prev[col]
    return out
...
def main():
    ...
    keep_cols = ['Testtime[s]','SOC','Voltage[V]','Current[A]','Temperature[°C]','Q_c','dU_dt[V/s]','dI_dt[A/s]'] + [c for c in soh_cfg['model']['features'] if c in df.columns]
    ...
    freeze_cols = sorted(set(soh_cfg['model']['features'] + ['Voltage[V]','Current[A]','Temperature[°C]','Q_c','dU_dt[V/s]','dI_dt[A/s]']))
    df_gap = freeze_gap(df, gap_start, gap_end - gap_start, freeze_cols)
    ...
    clean_soh_hourly, clean_bins = predict_soh_hourly(df, base_features, interval_seconds, feature_aggs, soh_scaler, soh_model, device)
    ...
    df_clean_soc = df.copy(); df_clean_soc['SOH'] = expand_soh_to_rows(df, clean_bins, clean_soh_hourly, interval_seconds)
