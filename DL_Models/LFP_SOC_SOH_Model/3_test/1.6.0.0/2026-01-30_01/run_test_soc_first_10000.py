...
def main():
    ...
    # save outputs
    result_csv = os.path.join(out_dir, f"soc_pred_first_{args.n_rows}_rows_{cell}.csv")
    out_df = pd.DataFrame({
    ...
    try:
        ...
        ax.set_title(f"SOC Prediction – First {args.n_rows} Rows ({cell})")
        ax.set_xlabel('Index')
        ax.set_ylabel('SOC')
        ax.legend(loc='best')
        fig.tight_layout()
        plot_path = os.path.join(out_dir, f"soc_pred_first_{args.n_rows}_rows_{cell}.png")
