# -*- coding: utf-8 -*-
"""
Created on Thu Aug 24 08:53:50 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 18:26:54 2023
...
#Cell
path = r'c:\Users\Florian\SynologyDrive\TUB\3_Projekte\MG_Farm\4_Work_packages\3_State_estimation\01_LFP\00_Data\Versuch_18650\\'
path_store = r'c:\Users\Florian\SynologyDrive\TUB\3_Projekte\MG_Farm\4_Work_packages\3_State_estimation\01_LFP\00_Data\Versuch_18650_standart\\'
...
def read_data_from_files(path_data):
    ...
    # Falls keine .xls Dateien gefunden wurden, .xlsx Dateien suchen
    if not xls_dateien:
        xls_dateien = [os.path.join(path_data, datei) for datei in os.listdir(path_data) if datei.endswith('.xlsx')]
...
def process_data(data_raw, data_temp, data_dms, data_statis, columns):
    ...
    # Bestimmen, ob 'Step' oder 'Steps' in den DataFrames verwendet wird
    step_column_statis = 'Steps' if 'Steps' in data_statis.columns else 'Step'
    step_column_raw = 'Steps' if 'Steps' in data_raw.columns else 'Step'

    # Bestimmen, ob 'Raw Step ID' oder 'Original step' in data_statis verwendet wird
    step_id_column = 'Original step' if 'Original step' in data_statis.columns else 'Raw Step ID'

    # Raw Step ID oder Original step hinzufügen
    for step in data_statis[step_column_statis]:
        step_id = data_statis.loc[data_statis[step_column_statis] == step]
        if not step_id.empty and step_id_column in step_id.columns:
            data_raw.loc[data_raw[step_column_raw] == step, 'Raw Step ID'] = int(step_id[step_id_column].iloc[0])
...
def save_data(data, path_store, campaign, cell, cycle):
    ...
    df = pd.read_csv(path_store + 'CSV_Vorlage.csv', sep=";")
    ...
    save_path = os.path.join(path_store, campaign, campaign + '_' + cell)
...
for cell in cells:
    ...
    for cycle in cycles:
        ...
        csv_path = os.path.join(path_store, 'MGFarm_18650', f"MGFarm_18650_{cell}", csv_filename)
