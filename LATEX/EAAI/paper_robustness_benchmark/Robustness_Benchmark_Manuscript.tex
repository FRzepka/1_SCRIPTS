...
\begin{itemize}[leftmargin=*]
    ...
    \item The data-driven model (DD) represents a neural estimator pipeline in which SOC is predicted by a lightweight GRU-based sequence model while online SOH context is provided by a separate recurrent SOH estimator under the same causal execution constraints \cite{mondalEstimatingStateofChargeLithiumIon2024,yaoSmarterBatteryManagement2025b}.
\end{itemize}
...
\begin{figure*}[t]
    ...
    \caption{Planned methodology overview for the causal benchmark chain. The figure should summarize how measured battery signals are converted into online auxiliary features, processed by representative estimator classes, exposed to measurement-only disturbances, and finally evaluated through global and local performance metrics.}
    \label{fig:methodology_overview}
\end{figure*}

\subsection{SOC estimation methods}
\begin{itemize}[leftmargin=*]
    \item The direct-measurement SOC formulation used in DM and, in modified form, in HDM follows online Coulomb counting. The internal charge state is updated as
    \begin{equation}
        Q_m(k)=Q_m(k-1)+\frac{I_k \Delta t_k}{3600},
    \end{equation}
    and the SOC estimate is obtained as
    \begin{equation}
        \widehat{\mathrm{SOC}}(k)=\mathrm{clip}\!\left(1+\frac{Q_m(k)}{C},0,1\right),
    \end{equation}
    where $C$ denotes either a fixed nominal capacity or an SOH-adapted effective capacity.
    ...
    \item The hybrid ECM model uses an EKF-style state update with a state vector containing SOC and polarization states; the prediction step propagates current-dependent RC dynamics, and the correction step uses the voltage residual between the measured terminal voltage and the model-based estimate
    \begin{equation}
        \widehat{U}_k = \mathrm{OCV}(\widehat{\mathrm{SOC}}_k,\widehat{\mathrm{SOH}}_k,I_k) + V_{1,k} + V_{2,k} + R_i I_k
    \end{equation}
    to update the internal state.
    \item The data-driven SOC model performs rolling GRU inference on the feature vector \{Voltage[V], Current[A], Temperature[$^\circ$C], SOH, $Q_c$, $dU/dt$, $dI/dt$, $\Delta t$\}. The feature $Q_c$ is an online cumulative-charge proxy computed by the same current-integration principle that underlies the direct-measurement estimators,
    \begin{equation}
        Q_c(k)=Q_c(k-1)+\frac{I_k \Delta t_k}{3600},
    \end{equation}
    and is therefore used to provide the network with explicit charge-throughput information instead of leaving this relation implicit in the raw current stream alone.
...
\begin{figure}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_1_baseline_performance.png}{Baseline comparison figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_2_current_bias.png}{Current-bias sensitivity figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_3_noise_robustness.png}{Noise sensitivity figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_7_initial_state_recovery.png}{Initial-state recovery figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_4_signal_integrity.png}{Signal-integrity summary figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_5_missing_gap_transition.png}{Missing-gap transition figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_6_missing_gap_recovery.png}{Missing-gap recovery figure missing.}
...
\begin{figure*}[t]
    \centering
    \benchfig{../../../DL_Models/LFP_SOC_SOH_Model/4_simulation_environment/results/paper_figures_v2/Figure_8_spike_response.png}{Spike-response figure missing.}
