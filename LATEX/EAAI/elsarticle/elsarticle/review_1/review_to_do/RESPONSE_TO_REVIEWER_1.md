...
**Changes in the manuscript:** scope statement in the Abstract (page 1, lines 22--24); general discussion of transfer across cell chemistries (pages 39--40, lines 839--849); platform and scope discussion (page 40, lines 850--877); Conclusion and Outlook (pages 41--42, lines 878--918).
...
**Changes in the manuscript:** structured magnitude pruning and the explicit post-pruning fine-tuning boundary (pages 17--20, lines 388--441); post-training mixed-precision quantization details (pages 20--22, lines 442--481); Discussion of the compression boundary and training-aware comparison (page 40, lines 850--857); future work (page 42, lines 909--918).
...
**Changes in the manuscript:** pruning criterion and diagnostic scope (pages 18--19, lines 395--435); Error Distribution Analysis and removal of the causal regularisation claim (page 31, lines 644--661); Discussion (page 37, lines 762--780); Appendix A.1 (page 42, line 923) and Fig. A.16 (page 43).
...
**Changes in the manuscript:** Abstract (page 1, lines 17--21); measured kernel interval and build settings (page 25, lines 539--559); energy-proxy definition and interpretation (pages 25--27, lines 560--590); utility definition and sensitivity results (pages 34--35, lines 712--733); Discussion (pages 38--39, lines 815--838); Conclusion (page 41, lines 878--895). The revised KPI table captions and headings appear in Tables 1 and 2 on pages 26--27.
...
**Changes in the manuscript:** host-side temporal-analysis method (page 23, lines 493--504); Temporal Stability and Filter Interaction results (pages 29--30, lines 619--643); conclusion (pages 41--42, lines 896--908); Appendix A.2 and Fig. A.17 (page 44).
...
**Changes in the manuscript:** SOH prediction post-processing subsection (pages 14--15, lines 308--327); SOH Results text (pages 27--28, lines 609--618); compression/filter comparison (pages 29--30, lines 619--643); Discussion (page 37, lines 774--780); conclusion (pages 41--42, lines 896--908); Appendix A.3 and Fig. A.18 (page 45). The SOH dashboard caption appears in Fig. 11 on page 30.
...
**Changes in the manuscript:** revised utility equation, explicit equal weights, and sensitivity results (pages 34--35, lines 712--733); interpretation of the purpose and decision value of the score in the Discussion (page 39, lines 824--838); Appendix A.4 and Fig. A.19 (page 46).
...
**Response:** We added two deliberately limited software-level fault analyses. Randomly missed reported estimates are handled with hold-last. More importantly, a stateful input-buffer experiment now evaluates whether a transient fault propagates through the recurrent estimator. The exported C implementations of Base, Pruned, and Quantized process 8000 consecutive C07 samples at 1 Hz. For each task and model, 30 independent events are distributed over the sequence after at least 2048 clean samples. Ten events affect voltage, ten current, and ten temperature. At each event, the most significant FP32 mantissa bit is flipped in one input value before inference for exactly one sample. The clean and disturbed branches start from the same recurrent state, after which both receive the same 60 clean samples without resetting the LSTM state.

We report three complementary properties. The disturbed-clean peak quantifies the immediate fault effect, a sustained threshold determines recovery time, and the change in target MAE over the 61-sample window quantifies the resulting accuracy loss. This separation is necessary because a transient can accidentally move an imperfect prediction closer to the target and because a window MAE alone can hide one short peak. All SOC variants recover in all 30 events within 60 s. Their median peak deviations are 3.29--3.82 percentage points, P95 peak deviations are 16.18--23.81 percentage points, and P95 increases in window MAE are 1.42--1.59 percentage points. For SOH, median peaks are 0.83--1.59 percentage points and P95 peaks are 3.37--5.85 percentage points. The shares recovered within 60 s are 93.3% for Base, 90.0% for Pruned, and 86.7% for Quantized. SOH P95 increases in window MAE remain 0.053--0.122 percentage points. No non-finite output occurs in the evaluated events.
...
**Changes in the manuscript:** Limited Software-Level Fault Sensitivity method and MAE definition (page 24, lines 512--538); Limited Software Fault Sensitivity results (pages 36--37, lines 736--761); interpretation in the Discussion (page 40, lines 866--877) and Conclusion (pages 41--42, lines 896--908); Appendix A.5 and Fig. A.20 (page 47), with the model-specific target-MAE values in Table A.4 (page 48).
...
- Reproducible stateful input-bitflip benchmark: `DL_Models/LFP_LSTM_MLP/5_benchmark/PC/input_bitflip_recovery/`
