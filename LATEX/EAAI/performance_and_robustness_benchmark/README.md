...
"Performance and Robustness Benchmarking Across Battery State-of-Charge
Estimator Classes for Embedded Applications". This copy is prepared for
Engineering Applications of Artificial Intelligence and is independent of the
JES working directory.

## Contents

- `main.tex`: non-anonymized manuscript with author and funding information
- `main_anonymized.tex`: optional blinded build using the same scientific source
...
Engineering Applications of Artificial Intelligence uses single-anonymized
review, so `main.pdf` is the intended manuscript file. The anonymized build is
retained as a fallback if the editorial system requests a blinded file. Before
uploading, review the manual items listed in `submission/CHECKLIST.md`.
