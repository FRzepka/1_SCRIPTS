...
\section{Results}
The result section first establishes the clean-condition reference and then evaluates how this reference changes under measurement-side disturbances. SOC errors are reported on the normalized 0 to 1 SOC scale in the tables and figures because this is the native scale of the benchmark simulations. Where the values are introduced in the text, they are also translated into percentage points to make the physical magnitude immediately visible. For example, $\mathrm{MAE}=0.0024$ corresponds to \SI{0.24}{\percent}.
