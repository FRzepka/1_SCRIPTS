...
Artificial-intelligence-based estimation methods are attractive because battery behaviour is nonlinear and depends on ageing, load history, temperature, and operating point. A small error on a clean laboratory test set alone does not establish practical suitability. What constitutes a sufficiently small error depends on the estimated state, the uncertainty of its reference, and the operating decision supported by the estimate. A method that is useful in a BMS must additionally handle imperfect measurements, causal online execution, memory and timing limits, and the practical route from a trained model to firmware. The methodological novelty of this research lies in connecting these stages: battery-state estimation is examined from data representation and nominal accuracy through disturbed-operation behaviour to measured embedded execution, rather than as an isolated offline modelling task.
